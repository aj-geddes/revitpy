...
class NamedPipeConnection:
    ...
    async def open(self):
        ...
        try:
            ...
            # Open pipes
            self.read_pipe = open(read_pipe_path)
            ...
            raise BridgeConnectionError(
                "named_pipe", f"Failed to open pipe connection: {e}"
            )
    ...
    async def receive(self, timeout: int | None = None) -> dict[str, Any] | None:
...
class NamedPipeServer:
    ...
    async def start(self):
        ...
        try:
            ...
            self.is_running = False
            raise BridgeConnectionError(
                "named_pipe", f"Failed to start pipe server: {e}"
            )
        ...
        try:
            ...
            while self.is_running:
                try:
                    ...
                    with open(control_pipe_path) as control_pipe:
                        while self.is_running:
    ...
    async def _handle_connection_messages(self, connection_id: str):
        ...
        while connection.is_connected and self.is_running:
            try:
                ...
                self.logger.error(
                    f"Error handling messages for connection {connection_id}: {e}"
                )
                break
    ...
    def get_active_connections(self) -> list[str]:
        ...
        return [
            conn_id for conn_id, conn in self.connections.items() if conn.is_connected
        ]
...
class NamedPipeClient:
    ...
    def __init__(self, pipe_name: str, connection_id: str | None = None):
        """Initialize pipe client."""
        self.pipe_name = pipe_name
        self.connection_id = connection_id or f"client_{int(time.time())}"
        self.is_connected = False
        self.base_pipe_path = os.path.join(
            tempfile.gettempdir(), f"revitpy_{pipe_name}"
        )
    ...
    async def connect(self, timeout: int = 10) -> bool:
        ...
        try:
            ...
            if not os.path.exists(control_pipe_path):
                raise BridgeConnectionError(
                    "named_pipe", "Control pipe not found. Server may not be running."
                )
    ...
    async def send_message(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Receive response
            read_pipe_path = f"{self.base_pipe_path}_read_{self.connection_id}"
            with open(read_pipe_path) as read_pipe:
