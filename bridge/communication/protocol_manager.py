...
class ProtocolManager:
    ...
    def __init__(self, config: CommunicationConfig):
        ...
        self.websocket_server: WebSocketBridgeServer | None = None
        ...
        self.default_message_handler: Callable | None = None

        # Statistics
        self.stats = {
            "messages_routed": {"named_pipe": 0, "websocket": 0, "file_exchange": 0},
            "errors": {"named_pipe": 0, "websocket": 0, "file_exchange": 0},
            "active_connections": {"named_pipe": 0, "websocket": 0},
        }
    ...
    async def start_all_protocols(self):
        ...
        if self.enabled_protocols[ProtocolType.WEBSOCKET]:
            try:
                ...
                self.websocket_server.set_message_handler(
        ...
        if startup_tasks:
            try:
                ...
                raise BridgeConnectionError(
                    "protocol_manager", f"Failed to start protocols: {e}"
                )
    ...
    async def _handle_pipe_message(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "protocol": "named_pipe"}
    ...
    async def _handle_websocket_message(
        ...
        try:
            ...
            return {"error": str(e), "protocol": "websocket"}

    async def broadcast_message(
        self,
        message: dict[str, Any],
        exclude_protocols: list[ProtocolType] | None = None,
    ) -> dict[str, int]:
        ...
        if (
            self.pipe_server
            and ProtocolType.NAMED_PIPE not in exclude_protocols
            and self.enabled_protocols[ProtocolType.NAMED_PIPE]
        ):
            try:
        ...
        if (
            ...
            and ProtocolType.WEBSOCKET not in exclude_protocols
            and self.enabled_protocols[ProtocolType.WEBSOCKET]
        ):
            try:
    ...
    async def send_to_protocol(
        self, protocol: ProtocolType, connection_id: str, message: dict[str, Any]
    ) -> bool:
        ...
        try:
            if protocol == ProtocolType.NAMED_PIPE and self.pipe_server:
                ...
                return await self.websocket_server.send_to_connection(
                    connection_id, message
                )

            else:
                self.logger.warning(
                    f"Protocol not available or not supported: {protocol}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Error sending to {protocol.value} connection {connection_id}: {e}"
            )
            return False

    async def create_file_exchange_request(
        self, request_data: dict[str, Any], file_id: str | None = None
    ) -> str:
    ...
    async def get_file_exchange_response(
        self, request_id: str
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            response_files = await self.file_handler.list_files(
                file_type="response", status="completed"
            )

            for response_file in response_files:
                if (
                    response_file.metadata
                    and response_file.metadata.get("request_id") == request_id
                ):
    ...
    def get_protocol_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "enabled_protocols": {
                p.value: enabled for p, enabled in self.enabled_protocols.items()
            },
        }
    ...
    def select_optimal_protocol(
        self, message_size: int, priority: str = "balanced"
    ) -> ProtocolType:
