...
class GeometrySerializer:
    ...
    def serialize_geometry(self, geometry: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            raise BridgeDataError("geometry_serialization", type(geometry).__name__, str(e))
        ...
        try:
            for geometry_obj in geometry_element:
                try:
                    ...
                    geometries.append({
                        'type': type(geometry_obj).__name__,
                        'error': str(e)
                    })
    ...
    def _serialize_solid(self, solid: Any) -> Dict[str, Any]:
        ...
        try:
            data = {
                'volume': round(solid.Volume, self.precision) if hasattr(solid, 'Volume') else None,
                'surface_area': round(solid.SurfaceArea, self.precision) if hasattr(solid, 'SurfaceArea') else None,
                'faces': [],
                'edges': []
            }
    ...
    def _serialize_face(self, face: Any) -> Dict[str, Any]:
        ...
        try:
            data = {
                'area': round(face.Area, self.precision) if hasattr(face, 'Area') else None,
                'material_element_id': face.MaterialElementId.IntegerValue if hasattr(face, 'MaterialElementId') else None
            }
    ...
    def serialize_curve(self, curve: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {
                ...
                'length': round(curve.Length, self.precision) if hasattr(curve, 'Length') else None
            }
    ...
    def _serialize_curve_loop(self, curve_loop: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'curves': curves,
                'count': len(curves),
                'is_open': curve_loop.IsOpen() if hasattr(curve_loop, 'IsOpen') else None
            }
                ...
                if hasattr(geometry, prop):
                    try:
                        ...
                        if isinstance(value, (int, float)):
                            data['properties'][prop.lower()] = round(value, self.precision)
                        elif hasattr(value, 'X'):  # Point or Vector
                            if 'center' in prop.lower() or 'origin' in prop.lower():
                                data['properties'][prop.lower()] = self._serialize_point(value)
                            else:
                                data['properties'][prop.lower()] = self._serialize_vector(value)
                    except:
                        continue
        ...
        try:
            ...
            if 'data' in geometry_data:
                ...
                if isinstance(data, dict):
                    ...
                    hash_props = ['volume', 'area', 'length', 'radius', 'center', 'origin']
