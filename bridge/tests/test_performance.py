...
class TestSerializationPerformance:
    ...
    def test_large_dataset_serialization(
        self, high_performance_serializer, performance_test_data
    ):
        """Test serializing large dataset (1000 elements)."""
        # Mock element extraction for performance test
        with patch.object(
            high_performance_serializer, "_extract_element_data"
        ) as mock_extract:
            ...
            serialized_elements = high_performance_serializer.serialize_elements(
                mock_elements
            )
            end_time = time.time()
    ...
    def test_streaming_serialization_performance(
        self, high_performance_serializer, performance_test_data
    ):
        ...
        with patch.object(
            ...
            start_time = time.time()
            batches = list(
                high_performance_serializer.serialize_elements_streaming(mock_elements)
            )
            end_time = time.time()
    ...
    def test_compression_efficiency(self, high_performance_serializer, temp_dir):
        ...
        for i in range(500):
            element = {
                ...
                "name": "Basic Wall - Type A",  # Repetitive
                "type": "Wall",
                "parameters": {
        ...
        assert (
            compressed_time < uncompressed_time * 2
...
class TestCommunicationPerformance:
    ...
    async def test_websocket_throughput(self, sample_analysis_request):
        ...
        assert all(
            len(client_results) == requests_per_client for client_results in results
        )

    @pytest.mark.asyncio
    async def test_concurrent_analysis_processing(
        self, mock_bridge_config, sample_analysis_request
    ):
        ...
        async def mock_analysis_handler(request):
            processing_time = (
                0.1 + (hash(request["analysis_id"]) % 100) / 1000
            )  # 0.1-0.2 seconds
        ...
        bridge_manager.analysis_handlers = {"energy_performance": mock_analysis_handler}
    ...
    def test_memory_usage_under_load(self, performance_test_data):
        ...
        config = ElementSerializationConfig(compression_enabled=True, batch_size=50)
...
class TestStressTests:
    ...
    def test_malformed_data_handling(self, high_performance_serializer=None):
        ...
        for malformed_input in malformed_inputs:
            try:
                result = high_performance_serializer.deserialize_element(
                    malformed_input
                )
