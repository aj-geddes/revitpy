...
from ..workflows.real_time_monitoring_workflow import RealTimeMonitoringWorkflow, SensorReading, MonitoringAlert
...
class TestBuildingPerformanceWorkflow:
    ...
    def test_element_selection_step(self, performance_workflow, mock_revit_elements):
        ...
        with patch.object(performance_workflow.element_selector, 'select_elements_interactively',
                         return_value=mock_revit_elements):
            elements, categories = performance_workflow._step_1_select_elements(interactive=False)
    ...
    def test_analysis_execution_step(self, performance_workflow, mock_revit_elements):
        ...
        with patch.object(performance_workflow.bridge, 'execute_analysis',
    ...
    def test_results_processing_step(self, performance_workflow):
        ...
        raw_results = {
            ...
            "results": {
                "overall_rating": "B+",
                "elements": {
                    "12345": {"rating": "A", "energy_consumption": 45.2}
                }
            }
        }
        
        processed = performance_workflow._step_3_process_results(raw_results, interactive=False)
        
        assert processed is not None
        assert "summary" in processed
        assert "recommendations" in processed
    
    def test_complete_workflow_execution(self, performance_workflow, mock_revit_elements):
        """Test complete workflow execution."""
        with patch.multiple(
            performance_workflow,
            _step_1_select_elements=Mock(return_value=(mock_revit_elements, ["Walls", "Windows"])),
            _step_2_execute_analysis=Mock(return_value={"success": True, "results": {}}),
    ...
    def test_error_handling_in_workflow(self, performance_workflow):
        ...
        with patch.object(performance_workflow, '_step_1_select_elements',
...
class TestSpaceOptimizationWorkflow:
    ...
    def test_space_analysis_step(self, optimization_workflow, mock_revit_elements):
        ...
        # Mock room elements
        room_elements = [elem for elem in mock_revit_elements if "Room" in str(elem.Category.Name)]
        
        with patch.object(optimization_workflow.element_selector, 'select_elements_by_category',
                         return_value=room_elements):
            spaces, furniture = optimization_workflow._step_1_analyze_current_layout(interactive=False)
    ...
    def test_genetic_algorithm_optimization(self, optimization_workflow):
        ...
        result = optimization_workflow._run_genetic_algorithm(mock_spaces, optimization_params)
    ...
    def test_simulated_annealing_optimization(self, optimization_workflow):
        ...
        result = optimization_workflow._run_simulated_annealing(mock_spaces, optimization_params)
    ...
    def test_layout_preview_generation(self, optimization_workflow):
        ...
        optimized_layout = {
            "spaces": [
                {"id": "room_1", "x": 0, "y": 0, "width": 10, "height": 10}
            ],
            "furniture": [
                {"id": "desk_1", "x": 2, "y": 2, "type": "desk"}
            ]
        }
    ...
    def test_complete_optimization_workflow(self, optimization_workflow, mock_revit_elements):
        """Test complete optimization workflow execution."""
        with patch.multiple(
            optimization_workflow,
            _step_1_analyze_current_layout=Mock(return_value=([], [])),
            _step_2_run_optimization=Mock(return_value={"efficiency_improvement": 0.15}),
            _step_3_generate_preview=Mock(return_value={"preview_generated": True})
        ):
            result = optimization_workflow.start_optimization_workflow(interactive=False)
            
            assert result["success"] is True
            assert "optimization_completed" in result or result.get("efficiency_improvement") is not None
...
class TestRealTimeMonitoringWorkflow:
    ...
    def test_sensor_configuration_step(self, monitoring_workflow, mock_revit_elements):
        ...
        elements, sensor_config = monitoring_workflow._step_1_configure_monitoring(interactive=False)
    ...
    def test_data_source_connection_step(self, monitoring_workflow):
        ...
        sensor_config = {
            "room_001": {
                "temp_sensor": {"type": "temperature", "location": "center"}
            }
        }
    ...
    def test_baseline_establishment_step(self, monitoring_workflow, mock_revit_elements):
    ...
    def test_realistic_reading_generation(self, monitoring_workflow):
        ...
        reading = monitoring_workflow._generate_realistic_reading("temp_001", "temperature")
    ...
    async def test_anomaly_detection(self, monitoring_workflow, sample_sensor_data):
        ...
        temp_alerts = [alert for alert in alerts if "temperature" in alert.description.lower()]
        assert len(temp_alerts) > 0
    
    @pytest.mark.asyncio
    async def test_parameter_updates(self, monitoring_workflow, mock_revit_elements, sample_sensor_data):
    ...
    @pytest.mark.asyncio
    async def test_background_monitoring(self, monitoring_workflow, mock_revit_elements):
        ...
        monitoring_task = asyncio.create_task(
            monitoring_workflow._run_background_monitoring(
                mock_revit_elements, sensor_config, duration=1  # 1 second
            )
        )
