...
class BuildingPerformanceWorkflow:
    ...
    def execute_complete_workflow(self, interactive: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if interactive:
                UI.TaskDialog.Show("Building Performance Analysis", 
            ...
            analysis_results = self._step_3_advanced_analysis(prepared_data, interactive)
            ...
            if interactive:
                UI.TaskDialog.Show("Workflow Complete",
            ...
            self.logger.info(f"Workflow completed successfully in {workflow_results['workflow_time']:.1f}s")
            ...
            if interactive:
                ...
                if elements:
                    ...
                    if not validation['valid']:
                        ...
                        UI.TaskDialog.Show("Selection Error", 
                                         f"Selected elements are not suitable for analysis:\n\n{error_msg}")
                        return []
                    
                    if validation['warnings']:
                        warning_msg = '\n'.join(validation['warnings'])
                        result = UI.TaskDialog.Show("Selection Warnings",
    ...
    def _step_2_prepare_data(self, elements: List[Any], interactive: bool) -> Dict[str, Any]:
        """Step 2: Prepare element data for analysis."""
        try:
            # Organize elements by category
            elements_by_category = self.element_selector.group_elements_by_category(elements)
            ...
            if interactive:
                ...
                prep_summary = f"Data preparation completed:\n\n"
                prep_summary += f"• Total elements: {len(elements)}\n"
                prep_summary += f"• Categories found: {', '.join(elements_by_category.keys())}\n"
    ...
    def _step_3_advanced_analysis(self, prepared_data: Dict[str, Any], 
        ...
        try:
            ...
            spaces_and_windows = [e for e in elements 
                                if any(cat in self._get_element_category(e).lower() 
    ...
    def _step_4_generate_recommendations(self, analysis_results: Dict[str, Any],
                                       elements: List[Any], 
                                       interactive: bool) -> List[Dict[str, Any]]:
        """Step 4: Generate optimization recommendations."""
        try:
            # Use ML-powered space optimization for comprehensive recommendations
            spaces = [e for e in elements if 'room' in self._get_element_category(e).lower() 
            ...
            if spaces:
                ...
                if space_optimization.get('optimized_layout'):
                    recommendations.extend(
                        self._convert_optimization_to_recommendations(space_optimization)
                    )
    ...
    def _step_5_display_results(self, analysis_results: Dict[str, Any], 
                               recommendations: List[Dict[str, Any]]):
        """Step 5: Display analysis results to user."""
        try:
            # Create comprehensive results summary
            results_summary = self._create_results_summary(analysis_results, recommendations)
            
            # Show results in formatted dialog
            BridgeUIHelpers.show_analysis_results(results_summary, "Building Performance Analysis")
            
            # Show detailed results for each analysis type
            for analysis_type, results in analysis_results.items():
                if results and isinstance(results, dict):
                    BridgeUIHelpers.show_analysis_results(results, f"{analysis_type.title()} Analysis")
    ...
    def _step_6_apply_improvements(self, recommendations: List[Dict[str, Any]],
                                  elements: List[Any], 
                                  interactive: bool) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for recommendation in selected_recommendations:
                try:
                    applied = self._apply_single_recommendation(recommendation, elements)
                    if applied:
            ...
            if interactive and improvements_applied:
                applied_count = len([i for i in improvements_applied if i['status'] == 'applied'])
                failed_count = len(improvements_applied) - applied_count
                
                status_msg = f"Improvement application completed:\n\n"
                status_msg += f"• Successfully applied: {applied_count}\n"
                status_msg += f"• Failed to apply: {failed_count}\n\n"
                
                if applied_count > 0:
                    status_msg += "The model has been updated with the applied improvements."
    ...
    def _step_7_generate_report(self, workflow_results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if PYREVIT_AVAILABLE:
                UI.TaskDialog.Show("Report Generated", 
    ...
    def _convert_optimization_to_recommendations(self, optimization_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Convert space optimization results to recommendations."""
        recommendations = []
        
        modifications = optimization_result.get('optimized_layout', {}).get('modifications_required', [])
    ...
    def _create_results_summary(self, analysis_results: Dict[str, Any], 
        ...
        if 'energy' in analysis_results:
            ...
            summary['key_findings']['energy'] = {
                ...
                'total_energy_usage': energy_data.get('energy_usage', {}).get('total_annual_kwh', 0),
                'rating': 'good' if energy_data.get('efficiency_rating', 0) > 0.7 else 'needs_improvement'
            }
        ...
        if 'thermal' in analysis_results:
            ...
            summary['key_findings']['thermal'] = {
                ...
                'thermal_bridges_found': thermal_data.get('thermal_bridging', {}).get('bridges_found', 0)
            }
        ...
        for priority in ['high', 'medium', 'low']:
            if by_priority[priority]:
                ...
                for i, rec in enumerate(by_priority[priority][:3]):  # Show top 3
                    summary_text += f"  {i+1}. {rec.get('description', 'Unknown recommendation')}\n"
    ...
    def _select_recommendations_to_apply(self, recommendations: List[Dict[str, Any]], 
                                        interactive: bool) -> List[Dict[str, Any]]:
        ...
        if high_priority:
            rec_list = '\n'.join([f"• {r.get('description', 'Unknown')}" for r in high_priority[:5]])
    ...
    def _apply_single_recommendation(self, recommendation: Dict[str, Any], 
        ...
        try:
            ...
            if rec_type == 'insulation_improvement':
                return self._apply_insulation_improvement(target_element, recommendation)
    ...
    def _apply_insulation_improvement(self, element, recommendation: Dict[str, Any]) -> bool:
        """Apply insulation improvement recommendation."""
        try:
            # In a real implementation, this would modify element parameters
            # For demonstration, we'll just log the action
            self.logger.info(f"Applied insulation improvement to element {self._get_element_id(element)}")
            return True
        except:
            return False
    ...
    def _apply_glazing_upgrade(self, element, recommendation: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Applied glazing upgrade to element {self._get_element_id(element)}")
            return True
        except:
            return False
    
    def _apply_space_modification(self, element, recommendation: Dict[str, Any]) -> bool:
        """Apply space modification recommendation."""
        try:
            # In a real implementation, this would modify space boundaries
            self.logger.info(f"Applied space modification to element {self._get_element_id(element)}")
            return True
        except:
            return False
