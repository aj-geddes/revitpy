...
# PyRevit imports
try:
    from pyrevit import UI, forms, revit
...
class SpaceOptimizationWorkflow:
    ...
    def execute_optimization_workflow(self, interactive: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            spaces, optimization_config = self._step_1_configure_optimization(
                interactive
            )
            ...
            self.logger.info(
                f"Space optimization workflow completed in {workflow_results['workflow_time']:.1f}s"
            )
            ...
            if interactive:
                UI.TaskDialog.Show(
                    "Workflow Error", f"Space optimization workflow failed:\n\n{str(e)}"
                )
    ...
    def _step_1_configure_optimization(self, interactive: bool) -> tuple:
        ...
        try:
            ...
            if interactive:
                ...
                spaces = self.element_selector.select_elements_by_category(
                    ["Rooms", "Spaces"]
                )
    ...
    def _step_2_define_constraints(
        self, spaces: list[Any], interactive: bool
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for space in spaces:
                ...
                space_constraints = {
                    "min_area": max(
                        current_area * 0.7, 10.0
                    ),  # Minimum 70% of current or 10m²
        ...
        try:
            ...
            optimization_results = self.bridge.execute_analysis(
                ...
                parameters={
                    ...
                    "algorithm": optimization_config.get(
            ...
            if not optimization_results or not optimization_results.get(
                "optimized_layout"
            ):
    ...
    def _step_4_generate_layouts(
        self, optimization_results: dict[str, Any], spaces: list[Any], interactive: bool
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            primary_layout = {
                ...
                "metrics": {
                    "efficiency_improvement": optimization_results.get(
                        "efficiency_improvement", 0
                    ),
                    "cost_impact": optimization_results.get("cost_impact", {}),
                    "fitness_score": optimization_results.get("fitness_score", 0),
                },
                "preview_data": self._generate_layout_preview_data(
                    optimized_spaces, spaces
                ),
            }
            ...
            if len(modifications) > 1:
                ...
                if conservative_modifications:
                    conservative_layout = {
                        ...
                        "spaces": self._apply_modifications_partially(
                            spaces, conservative_modifications
                        ),
                        "modifications": conservative_modifications,
                        "metrics": self._estimate_partial_metrics(
                            optimization_results, 0.7
                        ),
                        "preview_data": {},
                    }
            ...
            if interactive:
                layout_summary = (
                    f"Generated {len(layouts_generated)} layout options:\n\n"
                )
                for layout in layouts_generated:
                    improvement = layout["metrics"].get("efficiency_improvement", 0)
                    layout_summary += (
                        f"• {layout['name']}: {improvement:.1f}% improvement\n"
                    )
    ...
    def _step_5_preview_and_select(
        self,
        ...
        try:
            ...
            if selected_layouts:
                selection_summary = (
                    f"Selected {len(selected_layouts)} layout(s) for application:\n\n"
                )
                ...
                UI.TaskDialog.Show(
                    "No Selection", "No layouts were selected for application."
                )
    ...
    def _step_6_apply_optimizations(
        self,
                        ...
                        applied_optimizations.append(
                            {
                                ...
                                "modifications_applied": len(
                                    application_result["modifications_applied"]
                                ),
                                "modifications_failed": len(
                                    application_result["modifications_failed"]
                                ),
                                "application_time": application_result[
                                    "application_time"
                                ],
                            }
                        )
    ...
    def _step_7_validate_performance(
        self,
        ...
        if interactive and PYREVIT_AVAILABLE:
            try:
                ...
                selected_algorithm = forms.SelectFromList.show(
                    [
                        f"{key}: {desc}"
                        for key, desc in self.optimization_algorithms.items()
                    ],
                ...
                self.logger.warning(
                    f"Interactive configuration failed, using defaults: {e}"
                )
    ...
    def _refine_space_constraints(
        self, space, base_constraints: dict[str, Any], interactive: bool
    ) -> dict[str, Any]:
    ...
    def _show_constraints_summary(self, constraints: dict[str, Any]):
        ...
        summary = "Optimization Constraints Summary:\n\n"
        summary += "Global Constraints:\n"
        ...
        fixed_spaces = len(
            [c for c in space_constraints.values() if c.get("fixed_location", False)]
        )
        summary += "Space Constraints:\n"
    ...
    def _enhance_optimization_results(
        self,
        optimization_results: dict[str, Any],
        spaces: list[Any],
        optimization_config: dict[str, Any],
        ...
        optimized_spaces = optimization_results.get("optimized_layout", {}).get(
            "optimized_spaces", []
        )
        ...
        enhanced_results["area_analysis"] = {
            ...
            "area_change_percentage": (
                (optimized_total_area - original_total_area)
                / max(original_total_area, 0.001)
            )
            * 100,
        }
    ...
    def _generate_layout_preview_data(
        self, optimized_spaces: list[dict[str, Any]], original_spaces: list[Any]
    ) -> dict[str, Any]:
        ...
        for original_space in original_spaces:
            ...
            optimized_space = next(
                (s for s in optimized_spaces if s.get("id") == space_id), None
            )
                ...
                preview_data["space_changes"].append(
                    {
                        ...
                        "change_percentage": (area_change / max(original_area, 0.001))
                        * 100,
                    }
                )
    ...
    def _apply_modifications_partially(
        self, spaces: list[Any], modifications: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _estimate_partial_metrics(
        self, full_optimization_results: dict[str, Any], factor: float
    ) -> dict[str, Any]:
        """Estimate metrics for partial optimization."""
        original_improvement = full_optimization_results.get(
            "efficiency_improvement", 0
        )
    ...
    def _format_layout_details(self, layout: dict[str, Any]) -> str:
        ...
        details += "\nChanges Overview:\n"
        for i, modification in enumerate(
            layout.get("modifications", [])[:3]
        ):  # Show first 3
    ...
    def _apply_layout_modifications(
        self, layout: dict[str, Any], spaces: list[Any], interactive: bool
    ) -> dict[str, Any]:
                ...
                try:
                    ...
                    if target_space:
                        ...
                        modification_applied = {
                            ...
                            "modification_type": modification.get(
                        ...
                        self.logger.info(
                            f"Applied modification to space {target_space_id}"
                        )
                    else:
    ...
    def _show_application_results(self, applied_optimizations: list[dict[str, Any]]):
        ...
        successful = len(
            [opt for opt in applied_optimizations if opt["status"] == "applied"]
        )
        failed = len(applied_optimizations) - successful

        results_summary = "Optimization Application Results:\n\n"
        ...
        if successful > 0:
            ...
            for opt in applied_optimizations:
                if opt["status"] == "applied":
                    ...
                    results_summary += (
                        f"• {opt['layout_name']}: {modifications_applied} changes\n"
                    )
        ...
        if validation_results.get("validation_successful", False):
            ...
            if improvements:
                ...
                for improvement in improvements:
                    ...
                    validation_summary += (
                        f"• {layout_name}: {efficiency_gain:.1%} efficiency gain\n"
                    )
        else:
    ...
    def _show_completion_summary(self, workflow_results: dict[str, Any]):
        ...
        summary = "Space Optimization Workflow Completed!\n\n"
