...
from ..core.exceptions import CommandError, DevServerError
...
def server(
    ...
    websocket_port: int | None = typer.Option(None, "--ws-port", help="WebSocket port"),
    no_reload: bool = typer.Option(False, "--no-reload", help="Disable hot reload"),
    watch_patterns: list[str] | None = typer.Option(
        None, "--watch", help="Additional file patterns to watch"
    ),
    ignore_patterns: list[str] | None = typer.Option(
        None, "--ignore", help="File patterns to ignore"
    ),
) -> None:
    ...
    try:
        ...
        console.print("[bold green]🚀 Starting RevitPy development server[/bold green]")
...
def watch(
    ...
    patterns: list[str] | None = typer.Option(
        None, "--pattern", help="File patterns to watch"
    ),
    ignore_patterns: list[str] | None = typer.Option(
        None, "--ignore", help="Patterns to ignore"
    ),
    command: str | None = typer.Option(
        None, "--exec", help="Command to run on file changes"
    ),
) -> None:
...
class DevServer:
    ...
    def __init__(
        ...
        # Server components
        self.hot_reload_server: HotReloadServer | None = None
        self.websocket_server: WebSocketServer | None = None
        self.file_watcher: FileWatcher | None = None
        self.status_display: ServerStatusDisplay | None = None
    ...
    def _start_hot_reload(self) -> None:
        ...
        self.hot_reload_server = HotReloadServer(
            self.project_path, self.host, self.port
        )
...
class StandaloneWatcher:
    ...
    def _on_file_change(self, file_path: Path, event_type: str) -> None:
        ...
        """
        console.print(
            f"[yellow]{event_type}:[/yellow] {file_path.relative_to(self.project_path)}"
        )
...
class ProjectStatus:
    ...
    def _check_dependencies(self) -> dict:
        ...
        return {"healthy": True, "details": "Dependencies OK"}
    ...
    def _check_git_status(self) -> dict:
        ...
        if git_dir.exists():
            return {"healthy": True, "details": "Git repository initialized"}
        else:
            return {"healthy": False, "details": "Not a git repository"}
...
class ServerStatusDisplay:
    ...
    def _display_loop(self) -> None:
        """Main display loop."""
        with Live(
            self._generate_status(), refresh_per_second=1, console=console
        ) as live:
            self.live = live
            while self.running:
    ...
    def _generate_status(self) -> Panel:
        ...
        if self.server.hot_reload_enabled:
            ...
            table.add_row(
                "Connected Clients", str(len(self.server.clients)), "Active connections"
            )
        ...
        return Panel(
            table, title="[bold]RevitPy Development Server[/bold]", border_style="blue"
        )
