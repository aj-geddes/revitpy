...
from ..core.exceptions import DiagnosticsError, CommandError
...
def check(
    ...
    output_file: Optional[str] = typer.Option(None, "--output", help="Save report to file"),
) -> None:
    ...
    log_command_start("doctor check", {
        "verbose": verbose,
...
def performance(
    ...
    """
    start_time = time.time()
    log_command_start("doctor performance", {
        "benchmark": benchmark,
        "profile": profile,
    })
...
def cleanup(
    ...
    """
    start_time = time.time()
    log_command_start("doctor cleanup", {
        "cache": cache,
        "logs": logs,
        "temp": temp,
        "backup": backup,
    ...
    try:
        ...
        with Progress(
            ...
            task = progress.add_task("Cleaning files...", total=len(cleanup_plan["files"]))
...
class SystemDiagnostics:
    ...
    def check_dependencies(self) -> Dict[str, Any]:
        ...
        """
        required_packages = [
            "typer", "rich", "cookiecutter", "pydantic", "pyyaml",
            "watchdog", "websockets", "httpx", "packaging"
        ]
    ...
    def check_project_configuration(self) -> Dict[str, Any]:
        ...
        if (current_dir / "revitpy.toml").exists() or (current_dir / ".revitpy").exists():
    ...
    def display_results(self, results: Dict[str, Any]) -> None:
        ...
        py_table.add_row("Virtual Environment", "Yes" if py_info["is_virtual"] else "No")
        ...
        if revitpy_info["installed"]:
            revitpy_table.add_row("RevitPy Core", "✓ Installed", f"v{revitpy_info['version']}")
        else:
            revitpy_table.add_row("RevitPy Core", "✗ Not Found", "Install RevitPy package")
        
        if revitpy_info["cli_version"]:
            revitpy_table.add_row("RevitPy CLI", "✓ Installed", f"v{revitpy_info['cli_version']}")
        else:
    ...
    def fix_issues(self, results: Dict[str, Any]) -> int:
        ...
        if missing_deps:
            try:
                ...
                console.print(f"[green]✓[/green] Installed {len(missing_deps)} missing dependencies")
...
class PerformanceChecker:
    ...
    def display_performance_results(self, results: Dict[str, Any]) -> None:
        ...
        res_table.add_row("Memory", f"{resources['memory_percent']:.1f}% ({self.format_bytes(resources['memory_available'])} available)")
        res_table.add_row("Disk", f"{resources['disk_usage']['percent']:.1f}% ({self.format_bytes(resources['disk_usage']['free'])} free)")
    ...
    def display_profile_results(self, profile: Dict[str, Any]) -> None:
        ...
        prof_table.add_row("Memory RSS", self.format_bytes(profile["memory_info"]["rss"]))
        prof_table.add_row("Memory VMS", self.format_bytes(profile["memory_info"]["vms"]))
...
class SystemCleaner:
    ...
    def display_cleanup_plan(self, plan: Dict[str, Any]) -> None:
        ...
        console.print(f"[bold]Total size: {self.format_size(plan['total_size'])}[/bold]")
    ...
    def create_backup(self, files: List[Path]) -> Path:
        ...
        """
        backup_dir = Path.home() / ".revitpy" / "backups" / f"cleanup_{int(time.time())}"
