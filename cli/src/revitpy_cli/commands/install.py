...
from ..core.exceptions import InstallError, CommandError, DependencyError
...
@app.command()
def package(
    package_specs: List[str] = typer.Argument(..., help="Package specifications (name[==version])"),
    registry: Optional[str] = typer.Option(None, "--registry", "-r", help="Registry URL"),
    upgrade: bool = typer.Option(False, "--upgrade", "-U", help="Upgrade packages to latest versions"),
    ...
    force_reinstall: bool = typer.Option(False, "--force-reinstall", help="Force reinstall even if satisfied"),
    user: bool = typer.Option(False, "--user", help="Install to user site-packages"),
    target: Optional[str] = typer.Option(None, "--target", help="Install to specific directory"),
    editable: bool = typer.Option(False, "--editable", "-e", help="Install in editable mode (for local paths)"),
    requirements_file: Optional[str] = typer.Option(None, "--requirement", "-r", help="Install from requirements file"),
) -> None:
    ...
    log_command_start("install package", {
    ...
    try:
        ...
        # Success message
        console.print()
        console.print(f"[bold green]✓[/bold green] Successfully installed {len(install_result['installed'])} packages")
...
def uninstall(
    ...
    confirm: bool = typer.Option(True, "--confirm/--no-confirm", help="Confirm before uninstalling"),
    user: bool = typer.Option(False, "--user", help="Uninstall from user site-packages"),
) -> None:
    ...
    """
    start_time = time.time()
    log_command_start("install uninstall", {
        "packages": packages,
        "confirm": confirm,
        "user": user,
    })
    ...
    try:
        ...
        console.print(f"[bold red]🗑️  Uninstalling packages...[/bold red]")
        
        # Check which packages are installed
        installed_packages = uninstaller.check_installed_packages(packages, user_install=user)
        ...
        # Success message
        console.print()
        console.print(f"[bold green]✓[/bold green] Successfully uninstalled {len(uninstall_result['uninstalled'])} packages")
...
@app.command()
def list(
    format: str = typer.Option("table", "--format", help="Output format (table, json, freeze)"),
    outdated: bool = typer.Option(False, "--outdated", help="List outdated packages only"),
    user: bool = typer.Option(False, "--user", help="List user site-packages only"),
    show_dependencies: bool = typer.Option(False, "--show-dependencies", help="Show dependency tree"),
) -> None:
    ...
    """
    start_time = time.time()
    log_command_start("install list", {
        "format": format,
        "outdated": outdated,
        "user": user,
...
@app.command()
def show(
    package_name: str = typer.Argument(..., help="Package name to show information for"),
    registry: Optional[str] = typer.Option(None, "--registry", "-r", help="Registry URL"),
) -> None:
    ...
    """
    start_time = time.time()
    log_command_start("install show", {
...
def search(
    ...
    """
    start_time = time.time()
    log_command_start("install search", {
        "query": query,
        "registry": registry,
        "limit": limit,
    })
...
@app.command()
def update(
    packages: Optional[List[str]] = typer.Argument(None, help="Specific packages to update (default: all)"),
    pre: bool = typer.Option(False, "--pre", help="Include pre-release versions"),
    ...
    """
    start_time = time.time()
    log_command_start("install update", {
        "packages": packages,
        "pre": pre,
        "user": user,
        "dry_run": dry_run,
    ...
    try:
        ...
        # Success message
        console.print()
        console.print(f"[bold green]✓[/bold green] Successfully updated {len(update_result['updated'])} packages")
        
        # Show updated packages
        for pkg_info in update_result["updated"]:
            console.print(f"  📦 [cyan]{pkg_info['name']}[/cyan] {pkg_info['old_version']} → {pkg_info['new_version']}")
...
class PackageInstaller:
    ...
    def parse_requirements_file(self, requirements_file: Path) -> List[str]:
        ...
        if not requirements_file.exists():
            raise CommandError("install package", f"Requirements file not found: {requirements_file}")
    ...
    def install_packages(
        ...
        with Progress(
            ...
            task4 = progress.add_task("Downloading packages...", total=len(all_packages))
            downloaded_packages = []
            for i, pkg_spec in enumerate(all_packages):
                if not self._is_local_path(pkg_spec["name"]):
                    ...
                    downloaded_packages.append({"name": pkg_spec["name"], "path": pkg_spec["name"], "local": True})
                progress.update(task4, advance=1)
            
            # Step 5: Install packages
            task5 = progress.add_task("Installing packages...", total=len(downloaded_packages))
    ...
    def _parse_package_specs(self, packages: List[str]) -> List[Dict[str, Any]]:
        ...
        for pkg_spec in packages:
            if self._is_local_path(pkg_spec):
                ...
                try:
                    ...
                    raise InstallError(f"Invalid package specification '{pkg_spec}': {e}")
    ...
    def _is_local_path(self, package_spec: str) -> bool:
        ...
        """
        return (
            package_spec.startswith("./") or
            package_spec.startswith("../") or
            package_spec.startswith("/") or
            Path(package_spec).exists()
        )
    
    def _check_conflicts(self, packages: List[Dict[str, Any]], force: bool = False) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if package_response.status_code == 200:
                    ...
                    raise InstallError(f"Failed to download {name}: HTTP {package_response.status_code}")
            else:
...
class PackageUninstaller:
    ...
    def check_installed_packages(self, packages: List[str], user_install: bool = False) -> List[Dict[str, str]]:
        """Check which packages are installed.
    ...
    def uninstall_packages(self, packages: List[str], user_install: bool = False) -> Dict[str, Any]:
        """Uninstall packages.
...
class PackageLister:
    ...
    def get_installed_packages(self, user_only: bool = False, check_outdated: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if check_outdated:
                # Check for updates
                for pkg in packages:
                    pkg["outdated"] = self._check_package_outdated(pkg["name"], pkg["version"])
    ...
    def _check_package_outdated(self, name: str, current_version: str) -> bool:
        ...
        """
        try:
            response = self.client.get(f"{self.registry_url}/api/v1/packages/{name}/latest")
    ...
    def display_table(self, packages: List[Dict[str, Any]], show_dependencies: bool = False) -> None:
    ...
    def display_json(self, packages: List[Dict[str, Any]]) -> None:
    ...
    def display_freeze(self, packages: List[Dict[str, Any]]) -> None:
...
class PackageInfoDisplayer:
    ...
    def get_package_info(self, package_name: str) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.client.get(f"{self.registry_url}/api/v1/packages/{package_name}")
    ...
    def display_package_info(self, package_info: Dict[str, Any]) -> None:
        ...
        basic_table.add_row("Description", package_info.get("description", "No description"))
...
class PackageUpdater:
    ...
    def check_updates(
        ...
        for pkg in installed:
            try:
                response = self.client.get(f"{self.registry_url}/api/v1/packages/{pkg['name']}/latest")
    ...
    def display_available_updates(self, updates: List[Dict[str, Any]]) -> None:
    ...
    def update_packages(self, updates: List[Dict[str, Any]], user_install: bool = False) -> Dict[str, Any]:
        """Update packages.
...
class DependencyResolver:
    ...
    def resolve_dependencies(
        ...
        def resolve_recursive(pkg_spec: Dict[str, Any]) -> None:
            ...
            try:
                response = self.client.get(f"{self.registry_url}/api/v1/packages/{name}")
                ...
                if response.status_code == 200:
                    ...
                    for dep_spec in dependencies:
                        try:
                            ...
                            dep_pkg_spec = {
                                "name": dep_req.name,
                                "version": str(dep_req.specifier) if dep_req.specifier else None,
