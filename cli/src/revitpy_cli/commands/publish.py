...
from ..core.exceptions import PublishError, CommandError, AuthenticationError
...
@app.command()
def package(
    package_path: Optional[str] = typer.Argument(None, help="Path to package file"),
    ...
    sign: bool = typer.Option(True, "--sign/--no-sign", help="Sign package before publishing"),
    verify: bool = typer.Option(True, "--verify/--no-verify", help="Verify package after publishing"),
    repository: Optional[str] = typer.Option(None, "--repository", help="Repository name (for organization publishing)"),
) -> None:
    ...
    log_command_start("publish package", {
    ...
    try:
        ...
        # Success message
        if dry_run:
            console.print("[bold green]✓[/bold green] Package validation completed (dry run)")
        else:
            console.print("[bold green]✓[/bold green] Package published successfully!")
            console.print(f"[dim]Package URL: {publication_result['package_url']}[/dim]")
...
def login(
    ...
    token: Optional[str] = typer.Option(None, "--token", help="API token (alternative to username/password)"),
) -> None:
    ...
    """
    start_time = time.time()
    log_command_start("publish login", {
        "registry": registry,
        "username": username,
        "token": bool(token),
    })
    ...
    try:
        ...
        if auth_result["success"]:
            ...
            raise AuthenticationError("Login failed", auth_result.get("error", "Unknown error"))
...
def status(
    registry: Optional[str] = typer.Option(None, "--registry", "-r", help="Registry URL"),
    package: Optional[str] = typer.Option(None, "--package", help="Check specific package status"),
) -> None:
    ...
    try:
        ...
        with Progress(
            ...
            package_status = None
            if package:
                task3 = progress.add_task(f"Checking package '{package}'...", total=None)
...
def list_packages(
    registry: Optional[str] = typer.Option(None, "--registry", "-r", help="Registry URL"),
    user: Optional[str] = typer.Option(None, "--user", help="List packages for specific user"),
    limit: int = typer.Option(20, "--limit", help="Number of packages to show"),
    search: Optional[str] = typer.Option(None, "--search", help="Search packages"),
) -> None:
    ...
    """
    log_command_start("publish list-packages", {
        "registry": registry,
        "user": user,
        "limit": limit,
        "search": search,
...
def select_package_interactively(packages: List[Path]) -> Path:
    ...
    while True:
        choice = Prompt.ask(
            f"\nSelect package to publish (1-{len(packages)})",
            default="1"
        )
...
class PackagePublisher:
    ...
    def __init__(self, package_path: Path, registry_url: str, config: Any) -> None:
        ...
        self.client = httpx.Client(
            timeout=30.0,
            headers={"User-Agent": f"RevitPy-CLI/{config.version}"}
        )
    ...
    def publish(
        ...
        with Progress(
            ...
            if conflict_check["exists"]:
                if not Confirm.ask(f"Package {metadata['name']} {metadata['version']} already exists. Overwrite?"):
    ...
    def _validate_package(self) -> None:
        ...
        if self.package_path.suffix not in ['.whl', '.tar.gz']:
            raise PublishError(f"Unsupported package format: {self.package_path.suffix}")
    ...
    def _check_conflicts(self, metadata: Dict[str, Any], auth_token: str) -> Dict[str, Any]:
    ...
    def _verify_upload(self, package_id: str, auth_token: str) -> None:
        ...
        try:
            ...
            if package_info["status"] != "published":
                raise PublishError(f"Package not properly published: {package_info['status']}")
...
class AuthenticationManager:
    ...
    def interactive_login(self, registry_url: str, username: Optional[str] = None) -> Dict[str, str]:
    ...
    def authenticate(self, registry_url: str, credentials: Dict[str, str]) -> Dict[str, Any]:
    ...
    def get_stored_token(self, registry_url: str) -> Optional[str]:
...
class PublishStatusChecker:
    ...
    def display_status(
        ...
        package_status: Optional[Dict[str, Any]] = None,
    ) -> None:
