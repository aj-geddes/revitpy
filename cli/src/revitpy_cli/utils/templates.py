...
class TemplateManager:
    ...
    def __init__(self) -> None:
        ...
        self.cache_dir = Path(self.config.template.template_cache_dir or 
                             (self.config.cache_dir / "templates"))
    ...
    def add_template(self, source: str, name: Optional[str] = None, update: bool = False) -> None:
        ...
        if template_dir.exists() and not update:
            raise TemplateError(name, "Template already exists (use --update to overwrite)")
    ...
    def _get_builtin_template_path(self, name: str) -> Optional[Path]:
    ...
    def _download_template(self, name: str, source: str) -> Optional[Path]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                for template_path in possible_paths:
                    if template_path.exists() and self._is_valid_template(template_path):
                        # Copy to cache
        ...
        for metadata_file in metadata_files:
            if metadata_file.exists():
                try:
                    ...
                    logger.debug(f"Failed to load template metadata from {metadata_file}: {e}")
