...
class ProjectValidator:
    ...
    def validate_project_structure(self) -> list[dict[str, str]]:
        ...
        if not has_tests:
            issues.append({"type": "warning", "message": "No test directory found"})

        # Check for README
        readme_files = ["README.md", "README.rst", "README.txt"]
        has_readme = any(
            (self.project_path / filename).exists() for filename in readme_files
        )

        if not has_readme:
            issues.append({"type": "warning", "message": "No README file found"})

        # Check for license
        license_files = ["LICENSE", "LICENSE.txt", "LICENSE.md"]
        has_license = any(
            (self.project_path / filename).exists() for filename in license_files
        )

        if not has_license:
            issues.append({"type": "warning", "message": "No LICENSE file found"})
    ...
    def validate_pyproject_toml(self) -> list[dict[str, str]]:
        ...
        try:
            ...
            issues.append({"type": "error", "message": f"Invalid pyproject.toml: {e}"})
            return issues
        ...
        if not project:
            ...
            if version and not self._is_valid_version(version):
                issues.append(
                    {"type": "error", "message": f"Invalid version format: {version}"}
                )
            ...
            for dep in dependencies:
                if not self._is_valid_requirement(dep):
                    issues.append(
                        {"type": "error", "message": f"Invalid dependency: {dep}"}
                    )
