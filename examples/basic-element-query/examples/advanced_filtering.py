...
from filters import CustomElementFilter, ParameterFilter, GeometryFilter, CategoryFilter
...
def main():
    ...
    try:
        ...
        if not all_elements:
            print("No elements found in the model. Please ensure you have an active Revit document with elements.")
            return 1
        ...
        type_mark_filter.add_parameter_filter("Type Mark", "A", "contains", case_sensitive=False)
        ...
        exterior_walls_filter.add_parameter_filter("Usage", "Exterior", "contains", case_sensitive=False)
        exterior_walls_filter.add_parameter_filter("Level", "Level 1", "contains", case_sensitive=False)
        ...
        if structural_walls:
            ...
            sorted_numeric = sorted(numeric_params.items(), key=lambda x: abs(x[1]), reverse=True)
        ...
        print(f"   Method 1 (separate queries): {method1_total} elements in {method1_time:.4f}s")
        print(f"   Method 2 (combined filter): {method2_total} elements in {method2_time:.4f}s")
        ...
        if high_perf_elements:
            ...
            for fmt in formats:
                ...
                if success:
                    print(f"      Exported {len(export_data)} elements to {output_file}")
                else:
        ...
        small_elements, large_elements = custom_filter.filter_by_area_category(all_elements)
        ...
        print(f"    Total processing time: {final_stats['total_processing_time']:.3f} seconds")
