...
def process_element_batch(
    query_tool: ElementQueryTool, elements: list, batch_size: int = 100
) -> dict[str, Any]:
    """
    ...
    print(
        f"Processing {len(elements)} elements in {total_batches} batches of {batch_size}"
    )
    ...
    for batch_idx in range(0, len(elements), batch_size):
        ...
        batch_stats = {
            ...
            "elements_per_second": len(batch_results) / batch_time
            if batch_time > 0
            else 0,
        }
        ...
        results["memory_usage"].append({"batch": batch_num, "memory_mb": memory_usage})

        print(
            f"   Batch {batch_num} complete: {len(batch_results)} processed, "
...
def analyze_batch_performance(batch_results: dict[str, Any]) -> dict[str, Any]:
    ...
    analysis = {
        ...
        "average_elements_per_second": sum(elements_per_second)
        / len(elements_per_second),
        ...
        "overall_success_rate": (
            ...
            / (batch_results["total_processed"] + batch_results["total_errors"])
            * 100
        ),
        "memory_usage_range": {
            "min_mb": min(m["memory_mb"] for m in batch_results["memory_usage"]),
            "max_mb": max(m["memory_mb"] for m in batch_results["memory_usage"]),
            "avg_mb": sum(m["memory_mb"] for m in batch_results["memory_usage"])
            / len(batch_results["memory_usage"]),
        },
    }
    ...
    if analysis["average_success_rate"] < 95:
        recommendations.append(
            "Consider improving error handling - success rate is below 95%"
        )

    if analysis["max_batch_time"] > analysis["average_batch_time"] * 2:
        recommendations.append(
            "Some batches are significantly slower - consider dynamic batch sizing"
        )

    if (
        analysis["memory_usage_range"]["max_mb"]
        - analysis["memory_usage_range"]["min_mb"]
        > 100
    ):
        recommendations.append(
            "Memory usage varies significantly - consider memory cleanup between batches"
        )

    if analysis["average_elements_per_second"] < 10:
        recommendations.append(
            "Processing speed is low - consider optimizing element property access"
        )
...
def parallel_element_processing(
    query_tool: ElementQueryTool, elements: list, max_workers: int = 4
) -> dict[str, Any]:
    """
    ...
    print(
        "Note: This is for post-processing tasks only - Revit API is not thread-safe!"
    )
    ...
    try:
        report = {
            "summary": {
                ...
                "total_processing_time": batch_results["total_time"],
                "overall_success_rate": performance_analysis.get(
                    "overall_success_rate", 0
                ),
                "average_processing_speed": performance_analysis.get(
                    "average_elements_per_second", 0
                ),
            },
...
def main():
    ...
    try:
        ...
        print(
            f"   Success rate: {(batch_results['total_processed'] / len(all_elements) * 100):.1f}%"
        )
        ...
        print(
            f"   Average batch time: {performance_analysis['average_batch_time']:.3f}s"
        )
        print(
            f"   Processing speed: {performance_analysis['average_elements_per_second']:.1f} elements/second"
        )
        print(
            f"   Memory usage: {performance_analysis['memory_usage_range']['min_mb']:.1f} - "
        ...
        if filtered_elements:
            filtered_batch_results = process_element_batch(
                query_tool, filtered_elements, min(25, len(filtered_elements))
            )

            print(
                f"   Filtered batch processing: {filtered_batch_results['total_processed']} processed"
            )
        ...
        if len(all_elements) > 10:
            parallel_results = parallel_element_processing(
                query_tool, all_elements, max_workers=4
            )

            if "parallel_time" in parallel_results:
                print(
                    f"   Parallel processing time: {parallel_results['parallel_time']:.3f}s"
                )
                print(
                    f"   Sequential processing time: {parallel_results['sequential_time']:.3f}s"
                )
        ...
        if batch_results["processed_elements"]:
            ...
            if PANDAS_AVAILABLE and batch_results["processed_elements"]:
                try:
                    ...
                    print(
                        f"     Average parameters per element: {df['parameter_count'].mean():.1f}"
                    )
