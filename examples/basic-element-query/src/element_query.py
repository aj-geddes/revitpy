...
try:
    # RevitPy imports
    from revitpy import Element, FilteredElementCollector, RevitAPI
...
class ElementQueryTool:
    ...
    def __init__(self, log_level: str = "INFO", config_file: str | None = None):
        """
    ...
    def get_elements_by_category(
        self, category_name: str, include_types: bool = False
    ) -> list[Element]:
        """
        ...
        try:
            ...
            if include_types:
                ...
                collector = collector.of_category(
                    category_name
    ...
    def get_elements_by_ids(self, element_ids: list[int | str]) -> list[Element]:
        """
        ...
        self.logger.info(
            f"Retrieved {len(elements)} elements from {len(element_ids)} IDs"
        )
        return elements
    ...
    def display_element_properties(self, element: Element) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get properties for element {element.Id}: {str(e)}"
            )
            self.stats["errors_encountered"] += 1
            return {}

    def get_element_parameters(
        self, element: Element, parameter_names: list[str] | None = None
    ) -> dict[str, Any]:
        """
    ...
    def search_elements_by_name(
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(matching_elements)} elements matching '{name_pattern}'"
            )
        ...
        return {
            **self.stats,
            ...
            "error_rate": (
                ...
                / max(self.stats["elements_processed"], 1)
                * 100
            ),
        }
    ...
    def _get_element_type(self, element: Element) -> str | None:
    ...
    def _get_element_level(self, element: Element) -> str | None:
    ...
    def _get_element_location(self, element: Element) -> dict[str, float] | None:
        ...
        try:
            ...
            if location:
                ...
                if hasattr(location, "Point"):
                    ...
                    return {"type": "Point", "x": point.X, "y": point.Y, "z": point.Z}
    ...
    def _get_element_geometry_info(self, element: Element) -> dict[str, Any] | None:
    ...
    def _get_element_materials(self, element: Element) -> list[str] | None:
