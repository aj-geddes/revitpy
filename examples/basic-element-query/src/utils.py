...
def setup_logging(
    log_level: str = "INFO", log_file: str | None = None, format_style: str = "detailed"
) -> logging.Logger:
    """
...
def format_element_data(
    properties: dict[str, Any], format_type: str = "summary"
) -> str:
    """
    ...
    if format_type == "json":
        ...
        if location:
            if location.get("type") == "Point":
                ...
                lines.append(
                    f"Start: ({location['start']['x']:.2f}, {location['start']['y']:.2f}, {location['start']['z']:.2f})"
                )
                lines.append(
                    f"End: ({location['end']['x']:.2f}, {location['end']['y']:.2f}, {location['end']['z']:.2f})"
                )
        ...
        return (
            f"Element {properties.get('id')} ({properties.get('category')}) - "
            f"{properties.get('name', 'Unnamed')}"
        )


def export_to_file(
    data: list[dict] | dict, output_path: str | Path, file_format: str = "json"
) -> bool:
    """
...
def _export_to_xml(data: list[dict] | dict, output_path: Path):
...
class MockParameter:
    ...
    def __init__(self, name: str, value: Any):
        ...
        self._value = value
        self.HasValue = True
        self.StorageType = MockStorageType(
            "Double" if isinstance(value, (int, float)) else "String"
        )
    ...
    def AsElementId(self):
        return MockElementId(
            int(self._value) if isinstance(self._value, (int, float)) else -1
        )
