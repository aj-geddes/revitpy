...
from revitpy.orm.exceptions import QueryError, ValidationError
...
class RevitORMExamples:
    ...
    def basic_querying_example(self):
        ...
        with RevitContext(self.provider) as ctx:
            ...
            # Ordered queries
            walls_by_height = (
                ctx.all(WallElement).order_by(lambda w: w.height).to_list()
            )
            ...
            print(
                f"Statistics - Count: {wall_count}, Avg Height: {avg_height:.2f}, Max: {max_wall.height}"
            )
    ...
    def complex_querying_example(self):
        ...
        with RevitContext(self.provider) as ctx:
            ...
            for summary in wall_summaries[:3]:
                print(
                    f"  {summary['name']}: {summary['area']:.1f} sq ft, {summary['fire_rating']}hr rating"
                )
    ...
    def validation_example(self):
        ...
        try:
            ...
            print(
                f"Caught expected validation error: {list(e.validation_errors.keys())}"
            )
    ...
    def caching_example(self):
        ...
        with RevitContext(self.provider, config=config) as ctx:
            ...
            start_time = datetime.now()
            large_rooms_cached = ctx.where(
                RoomElement, lambda r: r.area > 200
            ).to_list()
            second_duration = (datetime.now() - start_time).total_seconds() * 1000

            print(
                f"Found {len(large_rooms_cached)} large rooms in {second_duration:.2f}ms"
            )
            ...
            if ctx.cache_statistics:
                ...
                print(
                    f"Cache statistics: {stats.hit_rate:.1f}% hit rate, {stats.hits} hits, {stats.misses} misses"
                )
    ...
    def error_handling_example(self):
        ...
        with RevitContext(self.provider) as ctx:
            ...
            try:
                ...
                nonexistent = ctx.single(
                    WallElement, lambda w: w.name == "NonExistent Wall"
                )
            except QueryError as e:
