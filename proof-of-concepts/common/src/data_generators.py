    ...
    for i, date in enumerate(dates):
        for hour in range(24):
            ...
            consumption = (base_consumption * 
                         ...
                         weekly_factor[i] * 
                         hourly_multiplier +
                         random.uniform(-50, 50))  # noise
            
            hourly_data.append({
                'timestamp': date.replace(hour=hour),
                'energy_consumption_kwh': max(consumption, 50),  # Minimum consumption
...
def generate_construction_photos_metadata(days: int = 30) -> List[Dict[str, Any]]:
    ...
    for day in range(0, days, 2):  # Photos every 2 days
        ...
        phase_index = min(int(progress_ratio * len(construction_phases)), 
                         len(construction_phases) - 1)
        ...
        for photo_num in range(random.randint(3, 8)):  # 3-8 photos per day
            photos_metadata.append({
                ...
                'camera_angle': random.choice(['north', 'south', 'east', 'west', 'overhead']),
                ...
                'elements_visible': random.sample([
                    'concrete_walls', 'steel_beams', 'rebar', 'formwork',
                    'hvac_ducts', 'electrical_conduit', 'plumbing',
                    'insulation', 'drywall', 'windows', 'roofing'
    ...
    for day in range(days):
        ...
        for hour in range(24):
            ...
            forecast_data.append({
                ...
                'precipitation': random.uniform(0, 0.5) if random.random() < 0.3 else 0,
