...
class MockRevitAPI:
    ...
    def _generate_mock_elements(self) -> list[RevitElement]:
        ...
        for floor in range(1, 13):  # 12 floors
            for i in range(random.randint(15, 25)):  # 15-25 spaces per floor
                ...
                element = RevitElement(
                    ...
                    location=Point3D(
                        i * 20 + width / 2, depth / 2, (floor - 1) * 12 + height / 2
                    ),
                )
                ...
                element.set_parameter(
                    "LightingLoad", round(random.uniform(0.8, 2.5), 2), "Double"
                )
                element.set_parameter(
                    "EquipmentLoad", round(random.uniform(0.5, 3.0), 2), "Double"
                )
                ...
                element.set_parameter(
                    "AnnualEnergyUse", round(random.uniform(2000, 8000), 2), "Double"
                )  # kWh
                element.set_parameter(
                    "CoolingLoad", round(random.uniform(5, 25), 2), "Double"
                )  # BTU/hr/sf
                element.set_parameter(
                    "HeatingLoad", round(random.uniform(15, 35), 2), "Double"
                )  # BTU/hr/sf
        ...
        for i in range(500):  # 500 walls
            wall_type = random.choice(
                ["Exterior", "Interior", "Partition", "Curtain Wall"]
            )
            element = RevitElement(
            ...
            element.set_parameter(
                "Thickness", random.uniform(4, 12), "Double"
            )  # inches
            element.set_parameter(
                "U-Value", round(random.uniform(0.05, 0.35), 3), "Double"
            )  # BTU/(hr·ft²·°F)
            element.set_parameter(
                "R-Value", round(1 / element.get_parameter("U-Value"), 2), "Double"
            )
        ...
        for i in range(50):
            ...
            element.set_parameter(
                "Capacity", random.uniform(50000, 500000), "Double"
            )  # BTU/hr
            element.set_parameter(
                "Efficiency", round(random.uniform(0.75, 0.95), 3), "Double"
            )
            element.set_parameter(
                "PowerConsumption", round(random.uniform(5, 150), 2), "Double"
            )  # kW
            element.set_parameter(
                "MaintenanceDate",
            ...
            element.set_parameter(
                "ServiceLife", random.randint(15, 25), "Integer"
            )  # years
        ...
        for i in range(200):  # 200 structural elements
            ...
            element.set_parameter(
                "Material", random.choice(["Steel", "Concrete", "Wood"]), "String"
            )
            element.set_parameter(
                "LoadCapacity", random.uniform(50000, 200000), "Double"
            )  # lbs
            element.set_parameter(
                "DeadLoad", random.uniform(1000, 5000), "Double"
            )  # lbs
            element.set_parameter(
                "LiveLoad", random.uniform(2000, 8000), "Double"
            )  # lbs
    ...
    def get_elements(
        self, category: str | ElementCategory = None, element_type: str = None
    ) -> list[RevitElement]:
    ...
    def get_elements_in_view(
        self, view_direction: str = None, location: Point3D = None
    ) -> list[RevitElement]:
...
def get_elements_in_view(
    view_direction: str = None, location: Point3D = None
) -> list[RevitElement]:
