...
class RPYXBuilder:
    ...
    def load_manifest(self, manifest_file: str | Path | None = None) -> RPYXManifest:
        ...
        if manifest_path.suffix.lower() == ".toml":
            ...
            if "project" in data:
                ...
                manifest_dict = {
                    ...
                    "author": project_data.get("authors", [{}])[0].get(
                        "name", "Unknown"
                    ),
                    "author_email": project_data.get("authors", [{}])[0].get(
                        "email", ""
                    ),
                    ...
                    "documentation_url": project_data.get("urls", {}).get(
                        "Documentation"
                    ),
                    "keywords": project_data.get("keywords", []),
                    "dependencies": [
                        {"name": dep} for dep in project_data.get("dependencies", [])
                    ],
                    **revitpy_data,
                }
            else:
    ...
    def validate_structure(self) -> list[str]:
        ...
        # Check entry points
        for _entry_point, script_path in self.manifest.entry_points.items():
        ...
        for pattern in sensitive_patterns:
            ...
            if matches:
                issues.append(
                    f"Potentially sensitive files found: {', '.join(str(m) for m in matches)}"
                )

        return issues

    def build_package(
        self, output_path: str | Path, include_dev: bool = False
    ) -> dict[str, Any]:
        ...
        with zipfile.ZipFile(
            ...
            zf.writestr(
                "rpyx-build.json", json.dumps(build_metadata, indent=2, default=str)
            )
    ...
    def validate_package(self) -> dict[str, Any]:
        ...
        try:
            ...
            with open(self.package_path, "rb") as f:
                package_data = f.read()
                validation_result["file_hash"] = hashlib.sha256(
                    package_data
                ).hexdigest()
            ...
            validation_result["valid"] = (
                len(
                    [
                        ...
                        if "Missing required" in issue or "Entry point" in issue
                    ]
                )
                == 0
            )
...
class RPYXValidator:
    ...
    def validate_compatibility(
        package_path: str | Path, revit_version: str
    ) -> dict[str, Any]:
        ...
        result = {"compatible": True, "warnings": [], "errors": [], "tested": False}
        ...
        if min_version:
            try:
                ...
                if packaging.version.parse(revit_version) < packaging.version.parse(
                    min_version
                ):
                    result["compatible"] = False
                    result["errors"].append(
                        f"Revit {revit_version} is below minimum required version {min_version}"
                    )
            except Exception:
        ...
        if max_version:
            try:
                ...
                if packaging.version.parse(revit_version) > packaging.version.parse(
                    ...
                    result["compatible"] = False
                    result["errors"].append(
                        f"Revit {revit_version} is above maximum supported version {max_version}"
                    )
            except Exception:
                result["warnings"].append("Could not parse version constraints")
        ...
        if revit_version in tested_versions:
            ...
            result["warnings"].append(
                f"Package not tested with Revit {revit_version}. Tested versions: {', '.join(tested_versions)}"
            )
        ...
        for issue in known_issues:
            if revit_version in issue.get("affected_versions", []):
                result["warnings"].append(
                    f"Known issue: {issue.get('description', 'Unspecified issue')}"
                )

        return result

    @staticmethod
    def security_scan(package_path: str | Path) -> dict[str, Any]:
...
def create_sample_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "dependencies": [{"name": "revitpy", "version": ">=2.0.0"}],
        "revit_dependencies": ["Autodesk.Revit.DB", "Autodesk.Revit.UI"],
