...
class DesktopPackageCache:
    ...
    def __init__(self, cache_dir: str = None, registry_url: str = "http://localhost:8000"):
        self.cache_dir = Path(cache_dir) if cache_dir else Path.home() / ".revitpy" / "cache"
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cache_package ON cache_entries(package_name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cache_accessed ON cache_entries(last_accessed)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_queue_status ON download_queue(status)")
    ...
    async def get_cached_package(self, package_name: str, version: str) -> Optional[CacheEntry]:
    ...
    async def cache_package(self, package_name: str, version: str, file_content: bytes, metadata: Dict) -> CacheEntry:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """INSERT OR REPLACE INTO cache_entries
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (entry.package_name, entry.version, entry.file_path, 
                 json.dumps(entry.metadata), entry.file_hash, entry.file_size,
                 entry.cached_at, entry.last_accessed, entry.access_count)
            )
        
        return entry
    
    async def download_and_cache(self, package_name: str, version: str) -> Optional[CacheEntry]:
        ...
        try:
            ...
            download_url = f"{self.registry_url}/api/packages/{normalized_name}/{version}/download"
            ...
            # Cache the package
            entry = await self.cache_package(package_name, version, file_content, metadata)
    ...
    async def get_package(self, package_name: str, version: str, offline_ok: bool = True) -> Optional[CacheEntry]:
    ...
    async def search_packages_offline(self, query: str, revit_version: Optional[str] = None) -> List[Dict]:
        ...
        for package_name, package_info in offline_metadata.package_index.items():
            ...
            if (query_lower in package_name.lower() or 
                query_lower in package_info.get("summary", "").lower() or
                query_lower in package_info.get("description", "").lower()):
                
                # Filter by Revit version if specified
                if revit_version:
                    compatibility = offline_metadata.compatibility_matrix.get(package_name, {})
    ...
    async def load_offline_metadata(self) -> Optional[OfflineMetadata]:
        """Load offline metadata from cache."""
        if not self.offline_metadata_path.exists():
            return None
        
        try:
            async with aiofiles.open(self.offline_metadata_path, "r") as f:
    ...
    async def sync_metadata(self) -> bool:
        ...
        try:
            ...
            for package in search_data.get("results", []):
                ...
                try:
                    ...
                    async with session.get(compat_url) as compat_response:
                        ...
                        compatibility_matrix[normalized_name] = compat_data.get("compatibility_matrix", {})
            ...
            async with aiofiles.open(self.offline_metadata_path, "w") as f:
                await f.write(json.dumps(asdict(offline_metadata), default=str, indent=2))
    ...
    async def cleanup_cache(self, max_size_mb: int = 500, max_age_days: int = 30) -> Dict[str, int]:
    ...
    async def preload_packages(self, package_list: List[Tuple[str, str]], max_concurrent: int = 5) -> Dict:
            ...
            async with semaphore:
                try:
                    ...
                    if entry:
                        ...
                        results["failed"].append((package_name, version, "Package not found"))
                except Exception as e:
