...
def create_environment(
    ctx, name: str, python_version: str, revit_version: str, packages: tuple
):
...
def clone_environment(ctx, source: str, target: str):
    ...
    try:
        with console.status(
            f"[bold blue]Cloning environment '{source}' to '{target}'..."
        ):
...
@click.option(
    "--upgrade", "-U", is_flag=True, help="Upgrade packages to latest versions"
)
...
def install_packages(
    ...
    if environment:
        ...
        if not target_env:
            console.print(
                "❌ No active environment. Please specify --environment or activate an environment.",
                style="red",
            )
            sys.exit(1)

    console.print(
        f"Installing packages in environment: {target_env.name}", style="blue"
    )

    if dry_run:
        console.print(
            "🔍 Dry run mode - showing what would be installed:", style="yellow"
        )
...
@click.pass_context
def uninstall_packages(ctx, packages: tuple, environment: str | None, yes: bool):
    """Uninstall packages."""
    venv_manager = VirtualEnvironmentManager()
    ...
    console.print(
        f"Uninstalling packages from environment: {target_env.name}", style="blue"
    )
    ...
    console.print(
        f"\n📦 Uninstalled {success_count}/{len(packages)} packages successfully.",
        style="blue",
    )
...
@click.option("--environment", "-e", help="Target environment name")
@click.pass_context
def list_packages(ctx, environment: str | None):
...
@click.option(
    "--list", "list_config", is_flag=True, help="List all configuration values"
)
@click.option(
    "--global", "global_config", is_flag=True, help="Use global configuration"
)
@click.pass_context
def config_command(
    ctx, key: str | None, value: str | None, list_config: bool, global_config: bool
):
