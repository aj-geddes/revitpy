...
class PackageSpec:
    ...
    optional_dependencies: Dict[str, Dict[str, str]] = field(default_factory=dict)  # extra -> {name: version_spec}
...
class DependencyResolver:
    ...
    def get_compatible_versions(
        ...
        compatible_versions = []
        spec = SpecifierSet(version_spec) if version_spec != "*" else SpecifierSet(">=0.0.0")
        
        for version_str, package_spec in self.available_packages[normalized_name].items():
            try:
                ...
                if exclude_prereleases and (version_obj.is_prerelease or package_spec.is_prerelease):
                    continue
        ...
        compatible_versions.sort(
            key=lambda p: Version(p.version),
            reverse=True
        )
    ...
    def resolve_dependencies(
        ...
        while to_resolve:
            ...
            if normalized_name in resolution_stack:
                cycle = resolution_stack[resolution_stack.index(normalized_name):] + [normalized_name]
            ...
            compatible_versions = self.get_compatible_versions(normalized_name, version_spec)
            ...
            for extra in package_extras:
                if extra in selected_spec.optional_dependencies:
                    for dep_name, dep_spec in selected_spec.optional_dependencies[extra].items():
                        to_resolve.append((dep_name, dep_spec, f"{normalized_name}[{extra}]"))
    ...
    def _calculate_installation_order(self, resolved_packages: Dict[str, PackageSpec]) -> List[str]:
        ...
        in_degree = {name: 0 for name in resolved_packages}
    ...
    def create_lock_file(self, resolution_result: ResolutionResult) -> Dict:
        ...
        lock_data = {
            ...
            "generated_at": packaging_version.__version__,  # Use packaging version as timestamp alternative
            "packages": {}
        }
    ...
    def resolve_from_lock_file(self, lock_data: Dict) -> ResolutionResult:
        ...
        installation_order = lock_data.get("installation_order", list(resolved_packages.keys()))
