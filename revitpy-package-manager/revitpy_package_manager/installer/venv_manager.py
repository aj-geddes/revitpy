...
class RevitPyEnvironment:
    ...
    def site_packages(self) -> Path:
        ...
        if os.name == "nt":  # Windows
            ...
            return (
                self.path / "lib" / f"python{'.'.join(version_info)}" / "site-packages"
            )
    ...
    def load_lock_file(self) -> dict | None:
    ...
    def run_command(self, command: list[str], **kwargs) -> subprocess.CompletedProcess:
        ...
        env["PATH"] = (
            f"{self.path / 'Scripts' if os.name == 'nt' else self.path / 'bin'}{os.pathsep}{env['PATH']}"
        )
...
class VirtualEnvironmentManager:
    ...
    def get_environment(self, name: str) -> RevitPyEnvironment | None:
    ...
    def create_environment(
        ...
        try:
            ...
            raise VirtualEnvironmentError(f"Failed to create environment: {e}") from e
        ...
        for package in base_packages:
            if not env.install_package(package, upgrade=True):
                raise VirtualEnvironmentError(
                    f"Failed to install base package: {package}"
                )
    ...
    def clone_environment(
        ...
        if not source_env:
            raise VirtualEnvironmentError(
                f"Source environment '{source_name}' not found"
            )

        if self.get_environment(target_name):
            raise VirtualEnvironmentError(
                f"Target environment '{target_name}' already exists"
            )
        ...
        for candidate in candidates:
            try:
                ...
                if result.returncode == 0:
                    ...
                    if output_version.startswith(version) or output_version.startswith(
                        f"{major}.{minor}"
                    ):
                        # Return full path
    ...
    def get_current_environment(self) -> RevitPyEnvironment | None:
