...
from ..schemas import (
    PackageResponse, UserResponse,
    ScanResultResponse, VulnerabilityReportResponse
)
...
async def list_all_packages(
    ...
    if not include_private:
        query = query.where(Package.is_private == False)
    
    if not include_unpublished:
        query = query.where(Package.is_published == True)
...
@router.get("/security/vulnerabilities", response_model=List[VulnerabilityReportResponse])
