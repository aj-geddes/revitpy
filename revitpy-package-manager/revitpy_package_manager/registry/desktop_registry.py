...
class DesktopPackageRegistry:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_packages_name ON packages(normalized_name)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_versions_package ON package_versions(package_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_versions_revit ON package_versions(min_revit_version, max_revit_version)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_stats_package ON download_stats(package_id)"
            )
    ...
    async def get_package_details(self, package_name: str) -> dict | None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            package = conn.execute(
                "SELECT * FROM packages WHERE normalized_name = ?", (normalized_name,)
            ).fetchone()
            ...
            return {"package": dict(package), "versions": [dict(v) for v in versions]}
    ...
    async def get_compatibility_matrix(self, package_name: str) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            package = conn.execute(
                "SELECT id FROM packages WHERE normalized_name = ?", (normalized_name,)
            ).fetchone()
            ...
            for version in versions:
                compatibility[version["version"]] = {
                    ...
                    "tested_versions": json.loads(
                        version["tested_revit_versions"] or "[]"
                    ),
    ...
    async def store_package(
        self, package_file: UploadFile, metadata: PackageMetadata
    ) -> dict:
        ...
        if not package_file.filename.endswith(".rpyx"):
            raise HTTPException(
                status_code=400, detail="Package must be in .rpyx format"
            )
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    metadata.installation_size,
                    metadata.performance_impact,
                    metadata.requires_admin,
                    (
                        metadata.revit_compatibility.min_version
                        if metadata.revit_compatibility
                        else None
                    ),
                    (
                        metadata.revit_compatibility.max_version
                        if metadata.revit_compatibility
                        else None
                    ),
                    (
                        json.dumps(metadata.revit_compatibility.tested_versions)
                        if metadata.revit_compatibility
                        else "[]"
                    ),
                    (
                        json.dumps(metadata.revit_compatibility.known_issues)
                        if metadata.revit_compatibility
                        else "[]"
                    ),
                    filename,
                    ...
                    json.dumps(metadata.revit_dependencies),
                    json.dumps(metadata.install_hooks),
                    json.dumps(metadata.uninstall_hooks),
...
class PackageSearchRequest(BaseModel):
    ...
    revit_version: str | None = Field(None, description="Filter by Revit version")
    package_type: str | None = Field(None, description="Filter by package type")
...
class PackageUploadRequest(BaseModel):
    ...
    requires_admin: bool = False
    min_revit_version: str | None = None
    max_revit_version: str | None = None
...
async def search_packages(
    ...
    return {
        ...
        "results": results,
        "total": len(results),
        "filters": {"revit_version": revit_version, "package_type": package_type},
    }
...
async def get_compatibility_matrix(package_name: str):
    ...
    return {"package": package_name, "compatibility_matrix": compatibility}
