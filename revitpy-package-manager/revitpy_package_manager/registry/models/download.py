...
class DownloadStats(Base):
    ...
    revit_version: Mapped[str | None] = mapped_column(String(50))
    platform: Mapped[str | None] = mapped_column(String(100))

    # Installation context
    installer_name: Mapped[str | None] = mapped_column(
        String(100)
    )  # pip, revitpy-install, etc.
    installer_version: Mapped[str | None] = mapped_column(String(50))

    # File information
    file_size: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    download_duration: Mapped[int | None] = mapped_column(Integer)  # milliseconds

    # Success tracking
    download_completed: Mapped[bool] = mapped_column(
