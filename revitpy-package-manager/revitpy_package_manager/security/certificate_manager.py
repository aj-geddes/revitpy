...
from cryptography.x509 import (
    Certificate, CertificateRevocationList, Name, NameAttribute,
    ExtendedKeyUsage, KeyUsage, BasicConstraints, SubjectAlternativeName,
    DNSName, RFC822Name, load_pem_x509_certificate, load_der_x509_certificate
)
...
class CertificateStore:
    ...
    def __init__(self, store_path: Path):
        ...
        for path in [self.ca_certs_path, self.trusted_certs_path, 
                     self.revoked_certs_path, self.crl_path]:
    ...
    def add_trusted_certificate(self, cert: Certificate, alias: str, purpose: str = "code-signing") -> str:
    ...
    def get_certificate(self, fingerprint: str) -> Optional[Certificate]:
    ...
    def get_ca_certificates(self) -> List[Certificate]:
        ...
        for cert_file in self.ca_certs_path.glob("*.pem"):
            with open(cert_file, 'rb') as f:
                certificates.append(x509.load_pem_x509_certificate(f.read(), default_backend()))
        return certificates
    ...
    def revoke_certificate(self, cert: Certificate, reason: str = "unspecified") -> None:
    ...
    def update_crl(self, crl_url: str) -> bool:
        ...
        try:
            ...
            crl_filename = f"{parsed_url.netloc}{parsed_url.path}".replace('/', '_').replace(':', '_')
            ...
            with httpx.Client() as client:
                ...
                if not crl_verified:
                    raise CertificateValidationError("CRL signature verification failed")
...
class X509PackageSigner:
    ...
    def sign_package_with_certificate(
        ...
        cert_fingerprint = self.certificate_store._get_certificate_fingerprint(certificate)
        ...
        return {
            ...
            'algorithm': 'Ed25519' if isinstance(private_key, ed25519.Ed25519PrivateKey) else 'RSA-PSS',
    ...
    def verify_package_signature(
        ...
        try:
            ...
            metadata_str = json.dumps(signature_info.get('metadata', {}), sort_keys=True)
            metadata_hash = hashlib.sha256(metadata_str.encode()).hexdigest()
            ...
            try:
                ...
                elif algorithm == 'RSA-PSS' and isinstance(public_key, rsa.RSAPublicKey):
                    public_key.verify(
                        signature,
                        sign_data,
                        rsa.PSS(
    ...
    def _validate_certificate(self, certificate: Certificate) -> None:
        ...
        try:
            ...
            if not key_usage.digital_signature:
                raise CertificateValidationError("Certificate does not allow digital signatures")
        ...
        try:
            ...
            if x509.oid.ExtendedKeyUsageOID.CODE_SIGNING not in ext_key_usage:
                raise CertificateValidationError("Certificate is not valid for code signing")
        except x509.ExtensionNotFound:
    ...
    def _verify_certificate_chain(self, certificate: Certificate) -> None:
        ...
        if not issuer_cert:
            raise CertificateValidationError("Certificate issuer not found in trusted CA store")
    ...
    def _create_timestamp(self, data: bytes, timestamp_url: str) -> Optional[Dict]:
