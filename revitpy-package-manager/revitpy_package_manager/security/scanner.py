...
class SecurityFinding:
    ...
    def __init__(
        ...
        self.finding_type = (
            finding_type  # vulnerability, malware, policy_violation, etc.
        )
        self.title = title
...
class SecurityScanResult:
    ...
    def complete_scan(self, error: str | None = None):
...
class StaticCodeScanner(BaseSecurityScanner):
    ...
    DANGEROUS_PATTERNS = [
        ...
        (
            re.compile(r"\bsubprocess\.call\([^)]*shell\s*=\s*True"),
            "high",
            "Shell Injection",
        ...
        (
            ...
            "high",
            "Command Injection",
            "os.system() calls are vulnerable to injection",
        ),
        (
...
class DependencyScanner(BaseSecurityScanner):
    ...
    async def _check_dependency_vulnerability(
        ...
        try:
            # Query OSV API
            query = {"package": {"ecosystem": "PyPI", "name": package_name}}

            if version != "unknown":
                query["version"] = version

            response = await client.post(
                f"{self.vulnerability_db_url}/query", json=query, timeout=10.0
            )
    ...
    def _extract_cve_id(self, vuln_data: dict) -> str | None:
...
class MalwareScanner(BaseSecurityScanner):
    ...
    async def _check_suspicious_files(
        self, package_path: Path, result: SecurityScanResult
    ):
        ...
        for py_file in python_files:
            try:
                ...
                obfuscation_indicators = [
                    (
                        len(re.findall(r"eval\s*\(", content)) > 3,
                        "Multiple eval() calls",
                    ),
                    (
                        len(re.findall(r"exec\s*\(", content)) > 2,
                        "Multiple exec() calls",
                    ),
                    (
                        len(re.findall(r"__import__\s*\(", content)) > 5,
                        "Excessive dynamic imports",
                    ),
                    (
                        len([line for line in content.splitlines() if len(line) > 200])
                        > 10,
                        "Many very long lines",
                    ),
                    (
                        "base64" in content.lower() and "decode" in content.lower(),
                        "Base64 decoding found",
                    ),
