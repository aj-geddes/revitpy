...
class PackageSigner:
    ...
    def sign_data(
        self,
        data: bytes,
        private_key: Ed25519PrivateKey | RSAPrivateKey
    ) -> bytes:
...
class PackageSignatureManager:
    ...
    def verify_package_signature(
        ...
        signature_info: Dict,
        public_key: bytes,
        trusted_fingerprints: Optional[set] = None
        ...
        try:
            ...
            if trusted_fingerprints and public_key_fingerprint not in trusted_fingerprints:
                return False, f"Public key fingerprint {public_key_fingerprint} is not trusted"
            
            # Verify the signature info matches our expectations
            if signature_info.get('algorithm') != self.algorithm:
                return False, f"Algorithm mismatch: expected {self.algorithm}, got {signature_info.get('algorithm')}"
            ...
            is_valid = self.signer.verify_signature(sign_data, signature, public_key_obj)
