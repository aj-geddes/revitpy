...
def check_dependencies() -> Dict[str, Any]:
    ...
    try:
        ...
        critical_vulns = [v for v in vulnerabilities if "CVSS" in v.get("description", "") and "9." in v.get("description", "")]
        high_vulns = [v for v in vulnerabilities if "CVSS" in v.get("description", "") and ("8." in v.get("description", "") or "7." in v.get("description", ""))]
        ...
        return {
            "status": "error",
            "message": f"Failed to parse pip-audit output: {e}"
        }
...
def check_bandit_security() -> Dict[str, Any]:
    ...
    if result["returncode"] not in [0, 1]:  # Bandit returns 1 when issues found
        return {
            "status": "error", 
            "message": f"Bandit failed: {result['stderr']}"
        }
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Failed to parse Bandit output: {e}"
        }
...
def check_jwt_security() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error", 
                "message": f"Failed to test JWT security: {e}"
            }
        finally:
            os.environ.clear()
        ...
        return {
            "status": "error",
            "message": f"Failed to check JWT configuration: {e}"
        }
    ...
    for root, dirs, files in os.walk("."):
        for file in files:
            ...
            try:
                ...
                if file.endswith(('.yaml', '.yml', '.json', '.toml', '.env')) and mode & 0o111:
...
def check_hardcoded_secrets() -> Dict[str, Any]:
    ...
    if result["success"] and result["stdout"]:
        ...
        for line in lines:
            ...
            if 'test_' not in line and 'example' not in line and '.env.example' not in line:
                # Check for actual hardcoded values (not placeholders)
                if any(bad in line.lower() for bad in ['change_me', 'your_secret', 'your-secret', 'test_secret']):
                    continue
