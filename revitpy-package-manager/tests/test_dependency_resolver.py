...
def test_package_spec_revit_compatibility():
    ...
    spec = PackageSpec(
        name="test-package", version="1.0.0", supported_revit_versions=["2024", "2025"]
    )
...
def test_resolver_initialization():
    ...
    resolver = DependencyResolver(
        python_version="3.11.0", revit_version="2025", allow_prereleases=True
    )
...
def test_version_conflict_detection():
    ...
    version_conflicts = [
        c for c in result.conflicts if c.type == ConflictType.VERSION_CONFLICT
    ]
...
def test_missing_dependency():
    ...
    assert not result.is_successful
    missing_conflicts = [
        c for c in result.conflicts if c.type == ConflictType.MISSING_DEPENDENCY
    ]
...
def test_optional_dependencies():
    ...
    spec_main = PackageSpec(
        name="main-package",
        version="1.0.0",
        optional_dependencies={"extra1": {"optional-package": ">=1.0.0"}},
    )
...
def test_prefer_installed_packages():
    ...
    assert result.is_successful
    assert (
        result.resolved_packages["test-package"].version == "1.0.0"
    )  # Prefers installed
...
def test_complex_dependency_tree():
    ...
    specs = [
        ...
        PackageSpec(name="lib1", version="1.0.0", dependencies={"common": ">=1.0.0"}),
        PackageSpec(
            name="lib2",
            version="1.0.0",
