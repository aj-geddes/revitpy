...
from .exceptions import TransactionError, PermissionError
...
class Transaction:
    ...
    def start(self) -> None:
        ...
        if self._status != TransactionStatus.NOT_STARTED:
            raise TransactionError(f"Transaction already started with status: {self._status}")
    ...
    def commit(self) -> None:
        ...
        if self._status != TransactionStatus.STARTED:
            raise TransactionError(f"Cannot commit transaction with status: {self._status}")
        ...
        try:
            ...
            if not success:
                raise TransactionError("Provider failed to commit transaction", self.name)
            ...
            for handler in self._commit_handlers:
                try:
                    ...
                    logger.warning(f"Commit handler failed for transaction {self.name}: {e}")
            
            logger.debug(f"Committed transaction: {self.name} (duration: {self.duration:.3f}s)")
    ...
    def rollback(self) -> None:
        ...
        if self._status not in (TransactionStatus.STARTED, TransactionStatus.FAILED):
            logger.warning(f"Attempting to rollback transaction with status: {self._status}")
            return
        ...
        try:
            ...
            for handler in self._rollback_handlers:
                try:
                    ...
                    logger.warning(f"Rollback handler failed for transaction {self.name}: {e}")
...
class TransactionGroup:
    ...
    def add_transaction(self, options: Optional[TransactionOptions] = None) -> Transaction:
    ...
    def start_all(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Started transaction group: {self.name} ({len(self._transactions)} transactions)")
            
        except Exception as e:
            self.rollback_all()
            raise TransactionError(f"Failed to start transaction group: {e}", self.name, e)
        ...
        try:
            ...
            for transaction in committed_transactions:
                try:
                    ...
                    logger.error(f"Failed to rollback transaction during group commit failure: {rollback_error}")
            
            self._status = TransactionStatus.FAILED
            raise TransactionError(f"Failed to commit transaction group: {e}", self.name, e)
    ...
    def rollback_all(self) -> None:
        ...
        for transaction in self._transactions:
            try:
                ...
                logger.error(f"Failed to rollback transaction in group {self.name}: {e}")
