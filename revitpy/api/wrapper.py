...
from .query import Query, QueryBuilder, IElementProvider
from .transaction import Transaction, TransactionGroup, ITransactionProvider, TransactionOptions
from .exceptions import RevitAPIError, ConnectionError, ModelError
...
class IRevitApplication(Protocol):
    ...
    def CreateDocument(self, template_path: Optional[str] = None) -> Any: ...
...
class IRevitDocument(Protocol):
    ...
    def GetElements(self, filter_criteria: Optional[Any] = None) -> List[IRevitElement]: ...
    
    def GetElement(self, element_id: Any) -> Optional[IRevitElement]: ...
...
class RevitDocumentProvider(IElementProvider, ITransactionProvider):
    ...
    def get_element_by_id(self, element_id: Any) -> Optional[Element]:
        """Get element by ID."""
        try:
            # Check cache first
...
class RevitAPI:
    ...
    def __init__(self, revit_application: Optional[IRevitApplication] = None) -> None:
        ...
        self._active_document: Optional[RevitDocumentProvider] = None
    ...
    @property
    def active_document(self) -> Optional[RevitDocumentProvider]:
    ...
    def connect(self, revit_application: Optional[IRevitApplication] = None) -> None:
    ...
    def create_document(self, template_path: Optional[str] = None) -> RevitDocumentProvider:
        ...
        try:
            ...
            logger.info(f"Created new document with template: {template_path or 'Default'}")
            return provider
    ...
    def get_document_info(self, provider: Optional[RevitDocumentProvider] = None) -> DocumentInfo:
    ...
    def save_document(self, provider: Optional[RevitDocumentProvider] = None) -> bool:
        """Save document."""
        doc_provider = provider or self.active_document
        if not doc_provider:
    ...
    def close_document(
        self, 
        provider: Optional[RevitDocumentProvider] = None,
    ...
    def transaction_group(self, name: Optional[str] = None) -> TransactionGroup:
    ...
    def delete_elements(self, elements: Union[Element, List[Element], ElementSet]) -> None:
        """Delete elements."""
        if not self.active_document:
            raise ConnectionError("No active document")
