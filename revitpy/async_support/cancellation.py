...
class CancellationTokenSource:
    ...
    def cancel_after(self, timeout: timedelta, reason: str | None = None) -> None:
    ...
    def _setup_timeout(self, reason: str | None = None) -> None:
    ...
    def check_cancellation():
        for token in tokens:
            if token.is_cancelled:
                combined_token._cancel(
                    f"Combined token cancelled due to: {token.reason}"
                )
                break
...
async def with_cancellation(
    ...
    try:
        ...
        # Wait for either the coroutine or cancellation
            ...
            [coro_task, monitor_task], return_when=asyncio.FIRST_COMPLETED
        )
...
class CancellationTokenContext:
    ...
    def __init__(
        self, timeout: timedelta | None = None, reason: str | None = None
    ) -> None:
        self._timeout = timeout
        self._reason = reason
        self._token_source: CancellationTokenSource | None = None
