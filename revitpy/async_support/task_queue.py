...
from .progress import ProgressReporter, ProgressReport, ProgressState
...
class Task(Generic[T]):
    ...
    def add_completion_callback(self, callback: Callable[[TaskResult[T]], None]) -> None:
...
class TaskQueue:
    ...
    def __init__(
        ...
        # Task storage
        self._pending_queue: asyncio.PriorityQueue = asyncio.PriorityQueue(maxsize=max_queue_size or 0)
    ...
    async def wait_for_task(self, task_id: str, timeout: Optional[float] = None) -> TaskResult[T]:
        ...
        while True:
            ...
            if (task_id not in self._running_tasks and 
    ...
    async def start(self) -> None:
        ...
        logger.info(f"Started task queue {self.name} with {self.max_concurrent_tasks} workers")
            ...
            while self._is_running:
                try:
                    ...
                    try:
                        ...
                        logger.debug(f"Task {task.name} completed with status {result.status}")
