...
from .types import EventType, EventData, EventResult, EventPriority
from .handlers import BaseEventHandler, AsyncEventHandler
from ..async_support.task_queue import TaskQueue, Task
...
class EventDispatcher:
    ...
    def __init__(
        ...
        self._async_handlers_semaphore = asyncio.Semaphore(max_concurrent_async_handlers)
    ...
    def register_handler(
        ...
        if event_types is None:
            ...
            for event_type in event_types:
                ...
                logger.debug(f"Registered handler {handler.name} for event type {event_type.value}")
        ...
        if event_types is None:
            ...
            for event_type in event_types:
                if handler in self._handlers[event_type]:
                    ...
                    logger.debug(f"Unregistered handler {handler.name} from event type {event_type.value}")
    ...
    def dispatch_event(self, event_data: EventData, immediate: bool = False) -> EventDispatchResult:
        """
        ...
        """
        if self._debug_mode:
            logger.debug(f"Dispatching event: {event_data.event_type.value} (ID: {event_data.event_id})")
        ...
        for filter_func in self._event_filters:
            if not filter_func(event_data):
                ...
                return EventDispatchResult(
                    ...
                    final_result=EventResult.CONTINUE
                )
    ...
    def _queue_event(self, event_data: EventData) -> EventDispatchResult:
        ...
        with self._queue_lock:
            if len(self._event_queue) >= self.max_queue_size:
                ...
                logger.warning(f"Event queue full, dropped event: {dropped_event.event_id}")
    ...
    async def dispatch_event_async(self, event_data: EventData) -> EventDispatchResult:
        ...
        """
        if self._debug_mode:
            logger.debug(f"Dispatching event async: {event_data.event_type.value} (ID: {event_data.event_id})")
    ...
    async def _process_event_async(self, event_data: EventData) -> EventDispatchResult:
        ...
        try:
            ...
            sync_handlers = [h for h in handlers if not isinstance(h, AsyncEventHandler)]
            ...
            if result.final_result == EventResult.CONTINUE and async_handlers:
                ...
                for handler in async_handlers:
                    ...
                    async def process_async_handler(h: AsyncEventHandler) -> EventResult:
                ...
                if async_tasks:
                    ...
                    for i, async_result in enumerate(async_results):
                        if isinstance(async_result, Exception):
                            ...
                            if async_result == EventResult.CANCEL and event_data.cancellable:
                                event_data.cancel()
                                result.was_cancelled = True
                                result.final_result = EventResult.CANCEL
    ...
    def get_handler_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for handler in all_handlers:
            stats[handler.name] = {
                ...
                'last_error': str(handler.metadata.last_error) if handler.metadata.last_error else None,
            }
