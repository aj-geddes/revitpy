...
from .types import EventType, EventData, EventResult, EventPriority, create_event_data
from .handlers import BaseEventHandler, AsyncEventHandler, CallableEventHandler, AsyncCallableEventHandler
...
class EventManager:
    ...
    def __init__(self) -> None:
        ...
        # Revit integration
        self._revit_event_bridge: Optional[Any] = None
        ...
        for name in dir(instance):
            ...
            if hasattr(method, '_is_event_handler'):
                ...
                if handler_obj:
                    ...
                    logger.debug(f"Registered class method handler: {instance.__class__.__name__}.{name}")
    ...
    def discover_handlers(self, paths: Optional[List[Path]] = None) -> int:
        """
        ...
        for name in dir(module):
            ...
            if inspect.isfunction(obj) and hasattr(obj, '_is_event_handler'):
                ...
                if handler:
                    self.register_handler(handler, event_types)
                    discovered_count += 1
                    logger.debug(f"Discovered function handler: {module.__name__}.{name}")
            ...
            # Check classes
                ...
                for method_name in dir(obj):
                    ...
                    if hasattr(method, '_is_event_handler'):
                        logger.debug(f"Found handler method in class {obj.__name__}.{method_name}")
