...
class DIContainer:
    ...
    def _analyze_dependencies(self, service_type: type) -> list[type]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to analyze dependencies for {service_type.__name__}: {e}"
            )
            return []
    ...
    def _instantiate_service(self, descriptor: ServiceDescriptor) -> Any:
        ...
        try:
            ...
            if descriptor.implementation_type:
                constructor_args = self._resolve_constructor_args(
    ...
    def _resolve_constructor_args(self, service_type: type) -> dict[str, Any]:
        ...
        try:
            ...
            for param_name, param in signature.parameters.items():
                ...
                if param_type == inspect.Parameter.empty:
                    if param.default == inspect.Parameter.empty:
                        raise ValueError(
                            f"Cannot resolve parameter {param_name} for {service_type.__name__}"
                        )
                ...
                try:
                    ...
                    if param.default == inspect.Parameter.empty:
                        raise ValueError(
                            f"Cannot resolve dependency {param_type.__name__} for {service_type.__name__}"
                        )
            ...
            logger.error(
                f"Failed to resolve constructor args for {service_type.__name__}: {e}"
            )
            raise
        ...
        try:
            ...
            for param_name, param in signature.parameters.items():
                ...
                if param_type == inspect.Parameter.empty:
                    if param.default == inspect.Parameter.empty:
                        raise ValueError(
                            f"Cannot resolve parameter {param_name} for {func.__name__}"
                        )
                    continue  # Use default value
                ...
                try:
                    ...
                    if param.default == inspect.Parameter.empty:
                        raise ValueError(
                            f"Cannot resolve dependency {param_type.__name__} for {func.__name__}"
                        )
    ...
    def is_registered(self, service_type: type) -> bool:
        ...
        return service_type in self._services or (
            self.parent and self.parent.is_registered(service_type)
        )
...
def singleton(service_type: type | None = None):
...
def transient(service_type: type | None = None):
...
def scoped(service_type: type | None = None):
...
_current_container: DIContainer | None = None
