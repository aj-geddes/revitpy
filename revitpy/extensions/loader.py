...
class ExtensionLoader:
        ...
        for extension_dir in directory.iterdir():
            if extension_dir.is_dir():
                ...
                if metadata_file.exists():
                    try:
                        metadata = await self._load_metadata_from_file(metadata_file, extension_dir)
    ...
    async def _discover_extension_classes(self, py_file: Path) -> List[ExtensionMetadata]:
        ...
        try:
            ...
            for name in dir(module):
                ...
                if (inspect.isclass(obj) and 
                    hasattr(obj, '_is_extension') and 
                    hasattr(obj, '_extension_metadata')):
    ...
    async def _load_extension_class(self, metadata: ExtensionMetadata) -> Optional[Type[Extension]]:
        ...
        if hasattr(metadata, '_extension_file'):
            return await self._load_class_from_file(metadata._extension_file, metadata.name)
        
        # Load from directory
        if hasattr(metadata, '_extension_directory'):
            return await self._load_class_from_directory(metadata._extension_directory, metadata.name)
        
        logger.error(f"No source found for extension {metadata.name}")
        return None
    
    async def _load_class_from_file(self, py_file: Path, extension_name: str) -> Optional[Type[Extension]]:
