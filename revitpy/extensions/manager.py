...
class ExtensionManager:
    ...
    def __new__(cls, config: Optional[ExtensionManagerConfig] = None) -> 'ExtensionManager':
    ...
    def __init__(self, config: Optional[ExtensionManagerConfig] = None) -> None:
    ...
    @classmethod
    def get_instance(cls, config: Optional[ExtensionManagerConfig] = None) -> 'ExtensionManager':
    ...
    async def discover_extensions(self) -> List[ExtensionMetadata]:
        ...
        logger.info(f"Discovery complete: {len(discovered_extensions)} extensions found")
    ...
    async def _resolve_dependencies(self, metadata: ExtensionMetadata) -> bool:
        ...
        for dependency in metadata.dependencies:
            if dependency not in self._extensions:
                ...
                if not success:
                    logger.error(f"Failed to resolve dependency {dependency} for {metadata.name}")
                    return False
    ...
    def get_extension(self, name_or_id: str) -> Optional[Extension]:
    ...
    async def _register_core_services(self) -> None:
        ...
        self.container.register_singleton(LifecycleManager, instance=self.lifecycle_manager)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for status in ExtensionStatus:
            extensions_by_status[status.value] = len(self.get_extensions_by_status(status))
        ...
        return {
            ...
            'extension_directories': [str(d) for d in self.config.extension_directories],
            'auto_load_enabled': self.config.auto_load_extensions,
            'auto_activate_enabled': self.config.auto_activate_extensions,
        }
    
    def get_extension_info(self, name_or_id: str) -> Optional[Dict[str, Any]]:
...
def get_extension_manager(config: Optional[ExtensionManagerConfig] = None) -> ExtensionManager:
...
def get_extension(name_or_id: str) -> Optional[Extension]:
