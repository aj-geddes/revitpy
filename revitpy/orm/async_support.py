...
class AsyncTransaction:
    ...
    def __init__(
        ...
        self._context: AsyncTransactionContext | None = None
    ...
    async def begin(self) -> None:
        ...
        async with self._lock:
            if self._context is not None and self._context.is_active:
                ...
                logger.debug(
                    f"Started nested transaction: {nested_context.transaction_id}"
                )
            else:
    ...
    async def commit(self) -> None:
        ...
        async with self._lock:
            if not self.is_active:
                raise TransactionError(
                    "No active transaction to commit", transaction_state="inactive"
                )
            ...
            try:
                if self._nested_transactions:
                    ...
                    logger.debug(
                        f"Committed nested transaction: {nested_context.transaction_id}"
                    )
                else:
        ...
        async with self._lock:
            ...
            try:
                if self._nested_transactions:
                    ...
                    logger.debug(
                        f"Rolled back nested transaction: {nested_context.transaction_id}"
                    )
                else:
    ...
    async def __aenter__(self) -> AsyncTransaction:
        ...
        if self._timeout_seconds:
            ...
            async def timeout_handler():
                ...
                if self.is_active:
                    logger.warning(
                        f"Transaction {self.transaction_id} timed out, rolling back"
                    )
                    await self.rollback()
        ...
        try:
            ...
            for i in range(0, len(operations), self._batch_size):
                ...
                batch_results = await asyncio.gather(
                    *[
                        self._process_single_operation(op, operation_handler)
                        for op in batch
                    ],
                ...
                for j, result in enumerate(batch_results):
                    ...
                    if isinstance(result, Exception):
                        failed_operations.append(
                            {"operation": operation, "error": str(result)}
                        )
                        self._error_count += 1
                    else:
            ...
            return {
                ...
                "operations_per_second": (self._processed_count + self._error_count)
                / processing_time
...
class AsyncRevitContext:
    ...
    def __init__(
        ...
        *,
        ...
        self._current_transaction: AsyncTransaction | None = None
    ...
    def is_in_transaction(self) -> bool:
        ...
        return (
            ...
            and self._current_transaction.is_active
        )
    ...
    async def get_all_async(self, element_type: type[T] | None = None) -> list[T]:
        ...
        try:
            ...
            raise AsyncOperationError(
                f"Failed to get elements: {e}", async_operation="get_all", cause=e
            )
        ...
        try:
            ...
            for change in changes:
                if change.state == ElementState.ADDED:
                    ...
                    operations.append(
                        BatchOperation(
                            operation_type=BatchOperationType.DELETE, entity=change
                        )
                    )
    ...
    async def transaction(
        self, auto_commit: bool = True, timeout_seconds: float | None = None
...
def async_transaction(auto_commit: bool = True, timeout_seconds: float | None = None):
    """Decorator for automatic async transaction management."""

    def decorator(func):
        @functools.wraps(func)
        async def wrapper(self, *args, **kwargs):
            if hasattr(self, "_context") and isinstance(
                self._context, AsyncRevitContext
            ):
