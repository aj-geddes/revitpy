...
from .types import CacheEntry, CacheKey, ElementId
...
class CacheConfiguration:
    ...
    default_ttl_seconds: int | None = 3600  # 1 hour default TTL
...
class MemoryCache(CacheBackend):
    ...
    def _evict_one(self) -> None:
        ...
        if self._config.eviction_policy == EvictionPolicy.LRU:
            ...
            key_str = min(self._cache.keys(), key=lambda k: self._cache[k].access_count)
...
class CacheManager:
    ...
    @property
    def statistics(self) -> CacheStatistics | None:
        ...
        with self._lock if self._lock else self._no_op():
            try:
                if isinstance(self._backend, MemoryCache):
                    ...
                    logger.debug(
                        f"Invalidated {len(invalidated_keys)} entries for dependency: {dependency}"
                    )
                ...
                logger.error(
                    f"Cache invalidate by dependency error for {dependency}: {e}"
                )
                return 0
    ...
    def invalidate_by_pattern(self, pattern: str) -> int:
        ...
        with self._lock if self._lock else self._no_op():
            try:
                ...
                logger.debug(
                    f"Invalidated {len(keys_to_delete)} entries matching pattern: {pattern}"
                )
    ...
    def remove_invalidation_callback(
        self, callback: Callable[[CacheKey], None]
    ) -> None:
...
def create_entity_cache_key(entity_type: str, entity_id: ElementId) -> CacheKey:
    ...
    return CacheKey(entity_type=entity_type, entity_id=entity_id)


def create_query_cache_key(entity_type: str, query_hash: str) -> CacheKey:
    """Create cache key for a query result."""
    return CacheKey(entity_type=entity_type, query_hash=query_hash)


def create_relationship_cache_key(
    entity_type: str, entity_id: ElementId, relationship_path: str
) -> CacheKey:
