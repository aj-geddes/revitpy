...
class EntityTracker:
    ...
    def is_dirty(self) -> bool:
        ...
        return (
            self.state != ElementState.UNCHANGED
            or bool(self.property_changes)
            or bool(self.relationship_changes)
        )
...
class ChangeTracker:
        ...
        with self._lock if self._lock else self._no_op():
            ...
            try:
                ...
                raise ChangeTrackingError(
                    "Failed to attach entity for tracking",
                    entity=entity,
    ...
    def track_relationship_change(
        ...
        with self._lock if self._lock else self._no_op():
            ...
            tracker.track_relationship_change(
                relationship_name, change_type, related_entity_id, related_entity_type
            )
            ...
            self._change_counter += 1
            logger.debug(
                f"Tracked relationship change: {entity_id}.{relationship_name}"
            )
    ...
    def accept_changes(self, entity_id: ElementId | None = None) -> None:
        ...
        with self._lock if self._lock else self._no_op():
            ...
            logger.debug(
                f"Accepted changes for {'all entities' if entity_id is None else entity_id}"
            )
    ...
    def reject_changes(self, entity_id: ElementId | None = None) -> None:
        ...
        with self._lock if self._lock else self._no_op():
            ...
            logger.debug(
                f"Rejected changes for {'all entities' if entity_id is None else entity_id}"
            )
    ...
    def create_batch_operation(
        ...
        return BatchOperation(
            operation_type=operation_type, entity=entity, properties=properties or {}
        )
    ...
    def add_batch_operation(self, operation: BatchOperation) -> None:
        ...
        with self._lock if self._lock else self._no_op():
            ...
            logger.debug(
                f"Added batch operation: {operation.operation_type} for {operation.entity}"
            )
    ...
    def remove_change_callback(
        self, callback: Callable[[PropertyChange], None]
    ) -> None:
