...
from typing import (
    ...
    Callable, Awaitable, AsyncIterator, Iterator, Tuple
)
...
from .types import (
    ElementFilter, SortCriteria, QueryExpression, CacheKey, CachePolicy,
    QueryMode, IQueryable, IAsyncQueryable, IElementProvider, ElementId,
    QueryPredicate, QuerySelector, QueryKeySelector, AsyncQueryPredicate
)
from .exceptions import QueryError, LazyLoadingError, AsyncOperationError
...
class LazyQueryExecutor(Generic[T]):
    ...
    def execute(self) -> List[T]:
        ...
        if self._query_plan.cache_strategy != CachePolicy.NONE:
            cache_key = CacheKey(
                entity_type=self._element_type.__name__ if self._element_type else "Element",
        ...
        try:
            ...
            for operation, details in self._query_plan.operations:
                current_results = self._apply_operation(operation, details, current_results)
            
            self._results = current_results
            self._is_executed = True
            
            # Cache results if enabled
            if (self._query_plan.cache_strategy != CachePolicy.NONE and 
                len(self._results) < 1000):  # Don't cache huge result sets
                cache_key = CacheKey(
                    entity_type=self._element_type.__name__ if self._element_type else "Element",
                    query_hash=self.query_hash
                )
    ...
    def _apply_operation(self, operation: str, details: Any, elements: List[T]) -> List[T]:
...
class QueryBuilder(Generic[T], IQueryable[T], IAsyncQueryable[T]):
    ...
    def order_by(self, key_selector: QueryKeySelector[T]) -> 'QueryBuilder[T]':
        ...
        new_builder = self._clone()
        new_builder._query_plan.add_operation('order_by', (key_selector, False), cost=3.0)
        return new_builder
    
    def order_by_descending(self, key_selector: QueryKeySelector[T]) -> 'QueryBuilder[T]':
        """Sort elements in descending order by key."""
        new_builder = self._clone()
        new_builder._query_plan.add_operation('order_by', (key_selector, True), cost=3.0)
        return new_builder
    ...
    def distinct(self, key_selector: Optional[QueryKeySelector[T]] = None) -> 'QueryBuilder[T]':
    ...
    async def single_async(self, predicate: Optional[QueryPredicate[T]] = None) -> T:
