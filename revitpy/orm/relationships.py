...
class Relationship(Generic[T, R], ABC):
    ...
    @abstractmethod
    def load(self, entity: T, force_reload: bool = False) -> R | list[R] | None:
        """Load related data for an entity."""
        pass

    @abstractmethod
    async def load_async(
        self, entity: T, force_reload: bool = False
    ) -> R | list[R] | None:
    ...
    def _should_use_cache(self, entity: T) -> bool:
        ...
        return (
            ...
            and self._get_entity_id(entity) is not None
        )
...
class OneToOneRelationship(Relationship[T, R]):
    ...
    def load(self, entity: T, force_reload: bool = False) -> R | None:
        ...
        try:
            ...
            logger.error(
                f"Failed to load one-to-one relationship {self._config.name}: {e}"
            )
    ...
    async def load_async(self, entity: T, force_reload: bool = False) -> R | None:
        ...
        async def _load():
            try:
                ...
                logger.error(
                    f"Failed to load one-to-one relationship {self._config.name} async: {e}"
                )
                raise RelationshipError(
...
class OneToManyRelationship(Relationship[T, list[R]]):
    ...
    def load(self, entity: T, force_reload: bool = False) -> list[R]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load one-to-many relationship {self._config.name}: {e}"
            )
            raise RelationshipError(
                f"Failed to load relationship {self._config.name}",
                relationship_name=self._config.name,
    ...
    async def load_async(self, entity: T, force_reload: bool = False) -> list[R]:
        ...
        async def _load():
            try:
                ...
                if not isinstance(related_collection, list):
                    related_collection = (
                        [related_collection] if related_collection else []
                    )
                ...
                logger.error(
                    f"Failed to load one-to-many relationship {self._config.name} async: {e}"
                )
                raise RelationshipError(
                    f"Failed to load relationship {self._config.name} asynchronously",
                    relationship_name=self._config.name,
...
class ManyToManyRelationship(Relationship[T, list[R]]):
    ...
    def load(self, entity: T, force_reload: bool = False) -> list[R]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load many-to-many relationship {self._config.name}: {e}"
            )
            raise RelationshipError(
                f"Failed to load relationship {self._config.name}",
                relationship_name=self._config.name,
    ...
    async def load_async(self, entity: T, force_reload: bool = False) -> list[R]:
        ...
        async def _load():
            try:
                ...
                logger.error(
                    f"Failed to load many-to-many relationship {self._config.name} async: {e}"
                )
                raise RelationshipError(
                    f"Failed to load relationship {self._config.name} asynchronously",
                    relationship_name=self._config.name,
...
class RelationshipManager:
    ...
    def __init__(
        self, loader: IRelationshipLoader, cache_manager: CacheManager | None = None
    ) -> None:
        self._loader = loader
        self._cache_manager = cache_manager or CacheManager()
    ...
    def register_one_to_one(
        ...
        *,
        ...
        if inverse_property:
            self._register_inverse(
                source_type, target_type, relationship_name, inverse_property
            )
    ...
    def register_one_to_many(
        ...
        *,
        ...
        inverse_property: str | None = None,
        load_strategy: LoadStrategy = LoadStrategy.LAZY,
        cascade: set[CascadeAction] | None = None,
        cache_enabled: bool = True,
        batch_size: int = 100,
    ) -> None:
    ...
    def get_relationship(
        self, entity_type: type, relationship_name: str
    ) -> Relationship | None:
        """Get a registered relationship."""
        return self._relationships.get(entity_type, {}).get(relationship_name)

    def load_relationship(
        self, entity: T, relationship_name: str, force_reload: bool = False
    ) -> Any | list[Any] | None:
    ...
    def get_registered_relationships(
    ...
    def _register_relationship(
        self, source_type: type, relationship: Relationship
    ) -> None:
