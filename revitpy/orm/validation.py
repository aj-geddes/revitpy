...
from .types import ElementId, ElementState
...
class BaseElement(BaseModel):
    ...
    name: str | None = Field(None, description="Element name", max_length=1000)
    category: str | None = Field(None, description="Element category", max_length=255)
    level_id: ElementId | None = Field(None, description="Level ID reference")
    family_name: str | None = Field(None, description="Family name", max_length=255)
    type_name: str | None = Field(None, description="Type name", max_length=255)
    ...
    state: ElementState = Field(
        default=ElementState.UNCHANGED, description="Entity state"
    )
...
class WallElement(BaseElement):
    ...
    area: float | None = Field(None, ge=0, description="Wall area in square feet")
    volume: float | None = Field(None, ge=0, description="Wall volume in cubic feet")

    # Wall-specific properties
    base_constraint: str | None = Field(None, description="Base constraint level")
    top_constraint: str | None = Field(None, description="Top constraint level")
    ...
    # Material and finish
    structural_material: str | None = Field(None, description="Structural material")
    finish_material_interior: str | None = Field(
        None, description="Interior finish material"
    )
    finish_material_exterior: str | None = Field(
        None, description="Exterior finish material"
    )

    # Analytical properties
    structural: bool = Field(default=False, description="Is structural element")
    fire_rating: int | None = Field(
        None, ge=0, le=4, description="Fire rating (0-4 hours)"
    )
...
class RoomElement(BaseElement):
    ...
    occupancy: int | None = Field(None, ge=0, description="Occupancy count")
    ceiling_height: float | None = Field(None, gt=0, description="Ceiling height")

    # Environmental properties
    temperature: float | None = Field(
        None, ge=-50, le=150, description="Temperature in Fahrenheit"
    )
    humidity: float | None = Field(
        None, ge=0, le=100, description="Humidity percentage"
    )
    air_flow_required: float | None = Field(
        None, ge=0, description="Required air flow in CFM"
    )
...
class DoorElement(BaseElement):
    ...
    material: str | None = Field(None, description="Door material")
    fire_rating: float | None = Field(
        None, ge=0, le=4, description="Fire rating in hours"
    )
    hardware_set: str | None = Field(None, description="Hardware set specification")
...
class WindowElement(BaseElement):
    ...
    glass_type: str | None = Field(None, description="Glass type specification")
    frame_material: str | None = Field(None, description="Frame material")
    u_factor: float | None = Field(
        None, gt=0, description="U-Factor for energy analysis"
    )
    solar_heat_gain: float | None = Field(
        None, ge=0, le=1, description="Solar Heat Gain Coefficient"
    )

    # Performance properties
    sound_transmission_class: int | None = Field(
        None, ge=0, le=100, description="STC rating"
    )
    energy_star_rated: bool = Field(
        default=False, description="Energy Star certification"
    )
        ...
        try:
            if self.constraint_type == ConstraintType.REQUIRED:
                ...
                if value is not None and not re.match(
                    self.constraint_value, str(value)
                ):
                    return False, self.error_message
...
class ElementValidator:
    ...
    def validate_element(
        self, element: BaseElement, strict: bool = None
    ) -> dict[str, list[str]]:
        """
    ...
    async def validate_element_async(
    ...
    def assert_valid(self, element: BaseElement) -> None:
        ...
        if errors:
            raise ORMValidationError(
                "Element validation failed", validation_errors=errors, entity=element
            )

    def get_element_schema(self, element_type: str) -> dict[str, Any] | None:
        ...
        if isinstance(element, WallElement):
            ...
            if element.area and element.height and element.length:
                ...
                if abs(element.area - calculated_area) > 0.1:
                    errors["area"] = [
                        f"Area {element.area} does not match calculated area {calculated_area:.2f}"
                    ]
            ...
            if element.volume and element.height and element.length and element.width:
                ...
                if abs(element.volume - calculated_volume) > 0.1:
                    errors["volume"] = [
                        f"Volume {element.volume} does not match calculated volume {calculated_volume:.2f}"
                    ]
        ...
        # Room validation
            ...
            if element.occupancy and element.area:
                ...
                if area_per_person < 50:  # Minimum area per person
                    errors["occupancy"] = [
                        "Occupancy too high for room area (min 50 sq ft per person)"
                    ]
    ...
    def remove_custom_rule(
        self, property_name: str, constraint_type: ConstraintType
    ) -> bool:
        ...
        self.custom_rules = [
            ...
            if not (
                ...
                and rule.constraint_type == constraint_type
            )
        ]
...
class TypeSafetyMixin:
    ...
    def validate_collection(
        self, objects: list[Any], expected_type: type[T]
    ) -> list[T]:
...
def create_window(
    id: ElementId, width: float, height: float, **kwargs
) -> WindowElement:
