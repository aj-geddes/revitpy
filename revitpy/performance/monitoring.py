...
class MetricsCollector:
    ...
    def get_metric_history(
        self, metric_name: str, time_window_seconds: int = 3600
    ) -> list[MetricSnapshot]:
...
class AlertingSystem:
    ...
    def start_alerting(self):
        ...
        self._alerting_thread = threading.Thread(
            target=self._alerting_worker, daemon=True
        )
    ...
    def add_threshold(self, threshold: PerformanceThreshold):
        ...
        logger.info(
            "Added threshold for %s: %s %s (warning: %s, critical: %s)",
        ...
        with self._lock:
            ...
            summary = {
                ...
                "unacknowledged_alerts": len(
                    [a for a in active_alerts if not a.acknowledged]
                ),
    ...
    def _evaluate_threshold(self, threshold: PerformanceThreshold):
        ...
        history = self.metrics_collector.get_metric_history(
            threshold.metric_name, threshold.evaluation_window_seconds
        )
    ...
    def _check_alert_auto_resolution(self):
        ...
        with self._lock:
            ...
            for alert_id, alert in self.active_alerts.items():
                ...
                # Get recent data
                history = self.metrics_collector.get_metric_history(
                    alert.metric_name, auto_resolve_window
                )
...
class PerformanceMonitor:
    ...
    def __init__(self, config: dict[str, Any] = None):
        ...
        # Trend analysis
        self.trend_analysis_interval = self.config.get(
            "trend_analysis_interval_seconds", 3600
        )
                ...
                if current_stats and threshold:
                    ...
                    if trend.average_change_rate != 0:
                        if threshold.comparison_operator == ">":
                            ...
                            ) / trend.average_change_rate
                        elif threshold.comparison_operator == "<":
                            threshold_value = threshold.warning_threshold
                            hours_to_threshold = (
                                current_value - threshold_value
                            ) / abs(trend.average_change_rate)
                        else:
                            continue
    ...
    def _handle_performance_alert(self, alert: PerformanceAlert):
        ...
        if (
            alert.metric_name == "process.memory.rss_mb"
            and alert.severity == "critical"
        ):
            if self.performance_optimizer:
                logger.info(
                    "Triggering automatic memory optimization for critical memory alert"
                )
                try:
    ...
    def _calculate_trend(
        self, metric_name: str, time_window_hours: int
        ...
        sum_y = sum(values)
        sum_xy = sum(t * v for t, v in zip(timestamps, values, strict=False))
        ...
        if slope > 0:
            trend_direction = (
                "degrading"
                if metric_name.endswith(".percent") or "latency" in metric_name
                else "improving"
            )
        elif slope < 0:
            trend_direction = (
    ...
    def _calculate_health_score(
        self, alert_summary: dict[str, Any], metric_stats: dict[str, Any]
    ) -> float:
    ...
    def _generate_performance_recommendations(
        ...
        if alert_summary.get("critical_alerts", 0) > 0:
            recommendations.append(
                "CRITICAL: Address critical performance alerts immediately"
            )

        if alert_summary.get("warning_alerts", 0) > 2:
            recommendations.append(
                "Multiple warning alerts detected - review system performance"
            )
        ...
        if degrading_trends:
            recommendations.append(
                f"Performance degradation detected in: {', '.join(degrading_trends[:3])}"
            )
