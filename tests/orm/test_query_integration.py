...
class MockElementProvider:
    ...
    async def get_element_by_id_async(self, element_id: Any) -> Any | None:
...
class TestQueryBuilderIntegration:
    ...
    def setup_method(self):
        ...
        self.walls = [
            WallElement(
                id=1, name="Wall 1", height=10, length=20, width=0.5, category="Walls"
            ),
            WallElement(
                id=2, name="Wall 2", height=8, length=15, width=0.4, category="Walls"
            ),
            WallElement(
                id=3, name="Wall 3", height=12, length=25, width=0.6, category="Walls"
            ),
        ]
    ...
    def test_query_rooms_with_filtering(self):
        ...
        query = QueryBuilder(self.provider, RoomElement, self.cache_manager)
    ...
    def test_query_with_chained_operations(self):
        ...
        result = (
            query.where(lambda w: w.width > 0.4)
    ...
    def test_query_with_projection(self):
        ...
        wall_info = query.select(
            lambda w: {"name": w.name, "height": w.height}
        ).to_list()
...
class TestElementSetIntegration:
    ...
    def test_element_set_batch_operations(self):
        ...
        element_set = ElementSet(
            self.walls.copy(), element_type=WallElement, lazy=False
        )
...
class TestPerformanceAndOptimization:
    ...
    def setup_method(self):
        ...
        cache_config = CacheConfiguration(max_size=5000, enable_statistics=True)
        self.cache_manager = CacheManager(cache_config)
    ...
    def test_lazy_vs_eager_evaluation(self):
        ...
        eager_query = QueryBuilder(
            self.provider, WallElement, self.cache_manager, QueryMode.EAGER
        )
