    ...
    def test_oversized_input_rejection(self, validator):
        ...
        with pytest.raises(RevitValidationError) as exc_info:
            validator.validate_string_input(
                "element_name", oversized_string, max_length=1000
            )
        ...
        for dangerous_file in dangerous_files:
            with pytest.raises(RevitSecurityError):
                validator.validate_file_extension(
                    "file_path", dangerous_file, allowed_extensions
                )
    ...
    def test_numeric_range_validation(self, validator):
        ...
        with pytest.raises(RevitValidationError):
            validator.validate_integer_range(
                "height", -100, min_value=0, max_value=10000
            )

        with pytest.raises(RevitValidationError):
            validator.validate_integer_range(
                "height", 50000, min_value=0, max_value=10000
            )

        # Test float ranges
        with pytest.raises(RevitValidationError):
            validator.validate_float_range(
                "width", -5.5, min_value=0.0, max_value=1000.0
            )

        with pytest.raises(RevitValidationError):
            validator.validate_float_range(
                "width", 2000.5, min_value=0.0, max_value=1000.0
            )
